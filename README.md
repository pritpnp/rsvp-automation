...
- **Public RSVP collection** — Zone pages POST to `netlify/functions/submit-rsvp.js`, which honors a per-zone on/off toggle in the Supabase `rsvp_settings` table and writes to the `rsvps` table as the single source of truth (Supabase-only; the legacy Google Sheet has been retired — there is no Sheet mirror or dual-write).
...
- **Automated scheduled jobs** — Five GitHub Actions workflows under `.github/workflows/` run the recurring work (cron, dispatch, and event-triggered). Listed below.
...
- **Supabase (Postgres + Storage)** — Backs everything stateful: `rsvps`, `rsvps_archive`, `rsvp_settings`, `events`, `vip_passes`, `managers`, `manager_sessions`, `builder_sessions`, `flyer_reviews`, `zone_events`, `telegram_upload_sessions`, `invite_lookup_attempts`, `invite_ip_whitelist`, `audit_log`, plus the `flyer-reviews` Storage bucket. All access is via `SUPABASE_SERVICE_KEY` from Node — RLS is effectively bypassed and the auth perimeter lives in the function layer. See Supabase Schema.
...
| Front-end image capture | Native Canvas 2D (`<canvas>` composite + `toDataURL`) — no `html2canvas` | n/a (hand-written render in-page) | `public/flyer-builder/index.html` |
...
| Google Sheets (legacy) | `googleapis` | scripts `^171.4.0`, functions `^140.0.0`, root `^140.0.0` | **Retired from the live path.** Referenced best-effort (optional) only by legacy tools: `restore-rsvps.js`, `backfill-rsvps.js`, `scripts/cleanup-past-rsvps.js`. Supabase is the source of truth |
...
- `netlify.toml` sets `command = "bash build.sh"` and `publish = "dist"`. The `dist/` directory is committed to git, so the build is essentially a refresh of the flyer-builder subtree (see Config & Build). `netlify.toml` also carries a build `ignore` command that **skips** the Netlify build entirely when a commit only touches paths that do not affect the deploy — see Build & Deployment.
...
CommonJS only (`"type": "commonjs"` in `netlify/functions/package.json`). Node 22 globals are leveraged in places — e.g. `crypto.randomUUID()` is used in `review-flyer.js` and `submit-rsvp.js` without an explicit `require('crypto')`. State-changing handlers additionally call the shared `netlify/functions/_audit.js` helper (`logAudit(...)`) after doing their work — see Auth & Security.

### Database (Supabase Postgres)

Single Supabase project. All server code uses the service-role key (`SUPABASE_SERVICE_KEY`) with `@supabase/supabase-js`. No anon-key data path actually serves traffic; a handful of files keep a defensive `SUPABASE_SERVICE_KEY || SUPABASE_ANON_KEY` fallback but the queries they run require service-role unless RLS policies are added. See Supabase Schema for tables. **Supabase is the sole source of truth for RSVPs** — the previously-mirrored Google Sheet has been retired (see the External APIs note below).
...
- `build.sh` is a short shell script that `cp -r`s `public/flyer-builder/{templates,preview-templates,fonts}/` and `public/flyer-builder/index.html` into `dist/flyer-builder/`. That is the entire frontend "build".
- Browser uses native `fetch`, `localStorage` (for `admin_token`, `admin_role`, `admin_permissions`, `admin_username`), `navigator.share`, `navigator.clipboard.write` with `ClipboardItem`, and the Canvas 2D API (`<canvas>` + `toDataURL`) for the flyer-builder capture (no `html2canvas`).
...
- Audit trail: `manager-auth.js` logs login success/failure and logout; ~12 state-changing functions and `telegram-webhook.js` (flyer approve/reject) record their action into the `audit_log` table via `_audit.js`. The actor is derived from the request's own auth credential. See Auth & Security.
...
| GitHub push (ambient) | `git push origin HEAD:main` via `child_process.execSync` (with `git pull --rebase --autostash -X theirs origin main` retry) | `scripts/automate.js` | Push regenerated `dist/` using the workflow's ambient `actions/checkout@v5` token (needs `permissions: contents: write` on the job) |
| Telegram Bot API | raw `fetch` to `https://api.telegram.org/bot<TOKEN>/...` | `late-rsvp.js`, `review-flyer.js`, `telegram-webhook.js`, `scripts/cleanup-past-rsvps.js`, `scripts/final-summary.js`, `scripts/summary.js` | `sendMessage`, `sendPhoto` (multipart), `getFile`, `answerCallbackQuery`, `editMessageText/Caption/ReplyMarkup` |
| Google Sheets API v4 (legacy / retired) | `googleapis` (`google.auth.GoogleAuth` from service account JSON, `google.sheets({version:'v4'})`) | **Not on the live path.** `netlify/functions/backfill-rsvps.js`, `restore-rsvps.js`; `scripts/cleanup-past-rsvps.js` — all best-effort / optional | Legacy one-shot + recovery tools only. The live RSVP read/write path is **Supabase-only**; the Sheet mirror has been retired. `scripts/final-summary.js` separately pulls the public CSV export of the retired Sheet |
...
- Client-side: no `html2canvas`. The flyer-builder composites the flyer directly with the Canvas 2D API (`FlyerRender.compositeFlyer` / `renderOG` draw the zone template PNG + text overlays). `sendForReview()` re-renders each artifact into a fresh **offscreen** `<canvas>` — the flyer at 1500px wide (`toDataURL('image/jpeg', 0.92)`) and the OG card at 2400px wide (`toDataURL('image/jpeg', 0.9)`) — and sends the two base64 JPEG strings as `imageBase64` / `ogBase64` in the `review-flyer` request body. The admin portal's own flyer upload path compresses images client-side (canvas, ≤2200px) before POSTing to stay under Netlify's ~6MB function-payload limit.

### Scheduling

All recurring work is GitHub Actions cron — there is no in-app scheduler, no Vercel cron, no Supabase Edge function. See Workflows section for the full schedule; the runners are pinned to Node 22 via `actions/setup-node@v5 { node-version: '22' }` and concurrency groups serialize overlapping runs where needed (the RSVP Automation workflow uses the concurrency group `rsvp-automation-deploy` with `cancel-in-progress: false`).
...
├── .github/workflows/           # 5 GitHub Actions YAMLs (see Workflows section)
...
├── netlify.toml                 # Build cmd, publish dir, function dir, build "ignore" rule, redirects, CORS headers
├── package.json                 # Root deps for GitHub Actions (Supabase, bcrypt, googleapis [legacy])
├── package-lock.json            # Lockfile for the root manifest
├── public/                      # Hand-edited HTML sources for the "static-builder" pages
└── scripts/                     # Long-running Node automation scripts run by Actions (incl. scripts/migrations/)
```
...
public/
├── admin/
│   └── index.html                # ~2,079-line admin/superadmin SPA (talks to /.netlify/functions/*)
├── flyer-builder/
...
### `netlify/functions/` — serverless backend (24 functions + shared `_audit` helper + manifest)

Every function is auto-mounted at `/.netlify/functions/<name>` (see `netlify.toml` `[functions] directory = "netlify/functions"`). They are CommonJS modules and share `netlify/functions/package.json` for their runtime deps. `_audit.js` is a shared helper module (not an endpoint — the leading underscore keeps it out of the mounted-function namespace).

```text
netlify/
├── functions/
│   ├── _audit.js                    # Shared fire-and-forget audit-log helper (exports logAudit)
│   ├── admin-passes.js              # VIP-pass CRUD for admin portal
│   ├── admin-rate-limit.js          # /invite IP rate-limit dashboard + whitelist
│   ├── backfill-rsvps.js            # Legacy one-shot: copy old Sheet rows into Supabase rsvps (best-effort)
...
│   ├── get-rsvps.js                 # Admin RSVP view (Supabase-only source of truth; PATCH, DELETE)
...
│   ├── manage-flyers.js             # Upload/delete flyer.jpg in pritpnp/rsvp-automation repo (top-level try/catch + 413 guard)
│   ├── manage-rsvp.js               # Toggle per-zone / global RSVP on/off
│   ├── manager-auth.js              # /login /logout /verify — issues 24h x-manager-token; logs auth to audit_log
│   ├── package.json                 # Runtime deps for Functions (Supabase, bcryptjs, googleapis [legacy])
│   ├── package-lock.json
│   ├── refresh-flyers.js            # Dispatch rsvp-automation.yml (optionally single zone)
│   ├── restore-rsvps.js             # Legacy: move rows from rsvps_archive back into rsvps (best-effort Sheet ref)
...
│   ├── submit-rsvp.js               # Public RSVP submission endpoint (Supabase-only)
│   ├── superadmin-audit-log.js      # GET, superadmin-only reader for the audit_log table
...
│   ├── telegram-webhook.js          # Telegram bot webhook (commands + flyer-review callbacks; logs approve/reject)
│   ├── trigger-redeploy.js          # Superadmin button to dispatch rsvp-automation.yml
│   ├── update-event-name.js         # Per-zone admin-overrideable event name (zone_events; clears flyer_hash on rename)
...
These are heavier than Functions: `automate.js` calls Anthropic's vision API and uses `sharp` for image processing; the others work against Supabase (with `final-summary.js` also pulling the retired Sheet's public CSV export). They get their own `package.json` so the lighter scripts (e.g. summary) don't pay for `sharp`'s native install. This directory also holds `migrations/`, the hand-run SQL migration files.

```text
scripts/
├── automate.js                  # OCR → generate dist/ → git push dist/ (pull --rebase --autostash -X theirs retry) → Netlify deploy
├── cleanup-past-rsvps.js        # Archive rsvps → rsvps_archive for events whose date is past (runs 4 AM NY); legacy best-effort Sheet ref
├── final-summary.js             # 9 PM ET final-headcount Telegram digest for today's events (reads the retired Sheet's public CSV export)
├── migrations/                  # One-off SQL migrations, run by hand in the Supabase SQL editor
│   ├── 2026-08-28-add-audit-log.sql   # Creates the audit_log table (RLS on, service-role only)
│   └── 2026-08-28-add-flyer-hash.sql  # Adds the zone_events.flyer_hash column
├── package.json                 # Scripts deps (anthropic-ai, supabase, googleapis [legacy], sharp, xlsx, ...)
├── package-lock.json
...
`scripts/migrations/` holds SQL that is applied out-of-band (not by any workflow) — run each file in the Supabase SQL editor once. `2026-08-28-add-audit-log.sql` creates the `audit_log` table (with RLS enabled, service-role only) that backs the audit trail; `2026-08-28-add-flyer-hash.sql` adds the `flyer_hash` column to `zone_events` used to anchor an admin's event-name override to a specific flyer (see Flyer Builder Flow / Per-Event Configuration). See Recreate From Scratch for exactly when to run them.

### `.github/workflows/` — 5 cron + dispatch workflows
...
├── rsvp-automation.yml           # Cron 0 8 * * * + push to main → automate.js (concurrency group rsvp-automation-deploy; commits deadlines.json)
...
- `POST /.netlify/functions/submit-rsvp` — fired when the user clicks Submit. Body is `{ zone: RSVP_ZONE, name, guests, eventName: RSVP_EVENT_NAME }`. The endpoint writes to Supabase `rsvps` only. On `200 { ok:true, id }` the card swaps to the thank-you panel. On `403` (RSVPs closed by the server-side toggle) the late-RSVP path activates.
...
**Auth state required:** none to view the page. After a successful login, the four `admin_*` keys above are written. The server records both successful and failed logins (and logout) into the `audit_log` table.
...
- **File:** `public/admin/index.html` (deployed to `dist/admin/index.html`). This is by far the largest single file in the project (~2,079 lines).
...
| 📜 Audit Log | superadmin only | `superadmin-audit-log` (GET) |
...
The **📜 Audit Log** tab (superadmin only) calls `GET /.netlify/functions/superadmin-audit-log` and renders one row per `audit_log` entry: when (relative "time ago", with the exact timestamp on hover), who (actor name, with a `super` pill for superadmins), the action, the target (plus any `details` JSON), a ✅/❌ result, and a **Device** column showing a parsed label such as "iPhone · Safari" derived from the stored `user_agent` (via `deviceLabel()`), with the raw IP + user-agent shown on hover.
...
GET     /.netlify/functions/superadmin-audit-log       (superadmin)
...
**Flyer upload behavior:** the Flyers tab compresses each selected image client-side (drawn to a `<canvas>`, downscaled to ≤2200px) before base64-encoding and POSTing to `manage-flyers`, so a phone photo stays under Netlify's ~6MB function-payload limit. `manage-flyers.js` wraps its whole handler in a try/catch and returns a friendly 413 guard when the payload is still too large.
...
- An **Advanced Settings** collapsible (only shown when the session's `allowAdvanced` is true, or always when running in standalone mode) that lets the user drag elements on the canvas, change per-element font size and line height, and dump a `flyer-layout.json` for calibration via `saveLog()`.

**Right pane — two tabs:**

1. **Flyer** — the portrait print canvas (`#flyer-canvas`). Text overlays (datetime, RSVP, host name, address, mahaprasad, plus the Satsang Sabha `ss_*` overlays) are composited on top of the zone PNG template at `yPct` fractions by `FlyerRender.compositeFlyer`.
2. **Preview** — the 1200×630 social-card layout (`#og-canvas`) using the matching preview template, rendered by `renderOG`.

Both on-screen canvases are covered by a tiled diagonal **"PREVIEW"** watermark overlay (draft-only — see Notable behavior).

A single **Send for Review** button at the bottom triggers the submission (`sendForReview()`).
...
- `POST /.netlify/functions/review-flyer` when the user clicks Send for Review (`sendForReview()`). Headers include `x-builder-session: <sessionId>`. Body is `{ zone, imageBase64, ogBase64, eventData }`. There is no `html2canvas`: `sendForReview()` composites each artifact into a fresh **offscreen** `<canvas>` — the flyer via `renderFlyer(1500, canvas)` exported as `toDataURL('image/jpeg', 0.92)`, and the OG card via `renderOG(2400, canvas)` exported as `toDataURL('image/jpeg', 0.9)` — then strips the `data:` prefix to send the two base64 JPEG strings. On `200 { ok:true, reviewId }` the browser navigates to `/flyer-builder/review-sent/`.
...
- The on-screen draft carries a tiled diagonal **"PREVIEW"** watermark: a `.preview-watermark` DOM overlay (a repeating inline-SVG background) sits over **both** the `#flyer-canvas` (portrait) and `#og-canvas` (landscape) previews. It is **draft-only** — because it is a sibling `<div>`, not canvas pixels, and `sendForReview()` exports from separate offscreen canvases via `toDataURL` (which captures only canvas pixels), the published `flyer.jpg` / `og.jpg` carry **no** watermark.
...
- Submitting the flyer produces **two independent** captures — a portrait flyer (offscreen `renderFlyer` at 1500px wide) and a landscape OG card (offscreen `renderOG` at 2400px wide), not one shared payload. Both are rendered above the deploy size cap so they downsample crisply.
...
All server-side logic lives in `netlify/functions/`. Functions are CommonJS modules exporting `exports.handler = async (event) => {...}` and are auto-mounted by Netlify at `/.netlify/functions/<filename-without-.js>`. They share a single `package.json` at `netlify/functions/package.json` (pinned at `@supabase/supabase-js ^2.101.1`, `bcryptjs ^2.4.3`, `googleapis ^140.0.0` — `googleapis` survives only for the two legacy Sheet tools below).

Every function follows the same general pattern: handle an `OPTIONS` preflight up front with a wide-open `Access-Control-Allow-Origin: *`, look up the caller's identity by reading either `x-admin-password` (raw superadmin secret) or `x-manager-token` (24-hour opaque token issued by `manager-auth.js` and stored in the `manager_sessions` table) — with the flyer-builder pages using a third credential, `x-builder-session` — enforce a per-route permission gate, then talk to Supabase using the service-role key from `SUPABASE_SERVICE_KEY`. State-changing handlers additionally `require('./_audit')` and fire a background `logAudit(...)` on success (see the Audit Logging cluster). The 24 request-handling functions (plus one shared, non-endpoint `_audit.js` helper) break into six clusters.

---

### RSVP & Passes

This is the largest cluster. RSVPs flow public → `submit-rsvp.js` (writes the Supabase `rsvps` table — the sole source of truth; the legacy Google Sheet mirror has been retired); `rsvp-status.js` and `late-rsvp.js` cover the public-facing on/off + late-submission paths; `get-rsvps.js`, `get-archived-rsvps.js`, `manage-rsvp.js` are the live admin side, while `restore-rsvps.js` and `backfill-rsvps.js` are legacy maintenance tools that still touch the retired Google Sheet on a best-effort/optional basis. The VIP-pass workflow uses `lookup-pass.js` (name+phone → pass id), `get-pass.js` (public render endpoint by id), and `admin-passes.js` (CRUD on `vip_passes`).

#### submit-rsvp

- **File:** `netlify/functions/submit-rsvp.js`
- **Purpose:** Accept an RSVP from a zone landing page, enforce the admin's RSVP on/off toggle server-side, and write to Supabase as the sole source of truth. The legacy Google Sheet mirror was retired — there is no Sheet write.
- **HTTP method:** `POST` (plus `OPTIONS` preflight); `405` for anything else.
- **Auth:** Public — no auth.
- **Request:** JSON body `{ zone, name, eventName, guests }`. `guests` is clamped to `Math.max(1, parseInt)` and rejected with 400 if `> 100` (`submit-rsvp.js:36`).
- **Response:** `200 { ok: true, id }`. `400` invalid JSON / missing name / a name with no letters / a name containing digits / `guests > 100`. `403 { error: "RSVPs are currently closed for this event." }` when either the `global` or zone-specific row in `rsvp_settings` is disabled. `500` Supabase insert failure. `503` if the settings read itself errors.
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`. (No Google vars — the Sheet path is gone.)
...
- **Notes:**
  - `id` is generated as `crypto.randomUUID()` and stored in both `rsvps.id` AND `rsvps.sheet_row_id` (lines 61 & 75). `sheet_row_id` is now just a legacy NAME for the row's own UUID, kept as a stable cross-key — it is the value the admin API still exposes as `powerapps_id`. Nothing about it implies a live Sheet or Power Apps.
  - Settings query selects BOTH `'global'` and the request zone in one `.in()` call (line 48); a missing row defaults to enabled (`!== false`).
  - Junk-name guard (lines 30–35): rejects names with no letters, or any digits at all — the recurring data-quality problem was people typing the guest count (e.g. "4 people") into the name box.
  - There is no longer any Sheet append or `googleapis` import in this function; a Supabase insert failure returns `500` so the user can retry (the RSVP is genuinely lost otherwise).
...
- **Purpose:** Admin RSVP view and edit. Supabase `rsvps` is the sole source of truth — the legacy Google Sheet mirror/merge/dual-write has been retired, so GET/PATCH/DELETE all read and write Supabase only.
...
- **Request:**
  ...
  - DELETE: `{ powerapps_id }`.
- **Response:**
  - GET `200` array of `{ zone, name, guests: String, submitted, powerapps_id }`, Supabase rows newest-first.
  - PATCH/DELETE `200 { success: true }`.
  - `401` unauthorized, `403` missing permission, `500` Supabase read failure.
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `ADMIN_PASSWORD`. (No Google Sheet vars.)
- **Supabase tables:** `manager_sessions` (select), `managers` (select), `rsvps` (select / update / delete), `audit_log` (via `logAudit`).
- **External APIs:** None.
- **Called by:** `public/admin/index.html` (`RSVPS_API`).
- **Notes:**
  - `powerapps_id` in the request/response is the row's `sheet_row_id` — which is just the row's own UUID under a legacy name. The field name is retained for backward compatibility with the admin UI; there is NO live PowerApps or Google Sheet integration behind it.
  - If `SUPABASE_SERVICE_KEY` is unset the function returns `500 "Service misconfigured"` rather than silently falling back to the anon key (lines 43–46).
  - If the Supabase read itself errors, returns `500` (line 68) — refuses to show partial data that would mislead a manager (there is no Sheet fallback anymore).
  - PATCH and DELETE key on `sheet_row_id` (lines 100 & 121) — the cross-store key.
  - Every successful PATCH/DELETE fires a background `logAudit` (`rsvp.edit` / `rsvp.delete`, lines 106 & 127).
...
- **Supabase tables:** `manager_sessions` (select), `rsvp_settings` (upsert with `onConflict: 'zone'`), `audit_log` (via `logAudit`).
- **External APIs:** None.
- **Called by:** `public/admin/index.html` Settings tab.
- **Notes:** `global` is a meta-zone — when disabled, `submit-rsvp.js` will short-circuit and reject every zone. A successful toggle fires `logAudit` (`rsvp.toggle`, line 51).

#### restore-rsvps

- **File:** `netlify/functions/restore-rsvps.js`
- **Purpose:** Legacy maintenance tool — the inverse of the cleanup job. Pull rows back from `rsvps_archive` into the live `rsvps` table, and (best-effort) re-append them to the retired Google Sheet if the optional Sheet env vars are still configured.
- **HTTP method:** `POST` only (plus `OPTIONS`).
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `ADMIN_PASSWORD`, plus the OPTIONAL legacy Sheet vars `GOOGLE_SHEET_ID`, `GOOG_SA_JSON`. Supabase restore is unaffected if they are absent — only the Sheet re-append is skipped.
- **Supabase tables:** `rsvps_archive` (select / delete), `rsvps` (upsert with `onConflict: 'id'`).
- **External APIs:** Google Sheets API v4 (legacy, best-effort only) — `values.get` on `responses!E:E` (dedup), `values.append` on `responses!A:E`.
...
- **Notes:**
  ...
  - The entire Sheet re-append is wrapped in a `try/catch` (lines 115–138): a Sheet failure — including missing `GOOG_SA_JSON` / `GOOGLE_SHEET_ID`, which the Sheet helper throws on — just logs `Sheet re-append failed` and continues. The Sheet is retired, so this step is purely a courtesy for any lingering downstream Sheet consumers.
  - Order is strict: upsert to `rsvps` first, then best-effort Sheet re-append (dedups against column E), then delete from `rsvps_archive`.

#### backfill-rsvps

- **File:** `netlify/functions/backfill-rsvps.js`
- **Purpose:** Legacy one-shot migration: copy every historical Google Sheet RSVP row into `rsvps`. This is what emptied the (now-retired) Sheet into Supabase in the first place; idempotent via `sheet_row_id` dedup. Kept for reference/re-runs only — the live path no longer writes to any Sheet.
- **HTTP method:** `POST` only (plus `OPTIONS`).
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `ADMIN_PASSWORD`, plus the legacy Sheet vars `GOOGLE_SHEET_ID`, `GOOG_SA_JSON` (this tool cannot run without them — it exists specifically to read the Sheet).
- **Supabase tables:** `rsvps` (select all `sheet_row_id` for dedup, then insert).
- **External APIs:** Google Sheets API v4 (legacy) — `values.get` on `responses!A:E` with scope `spreadsheets.readonly`.
- **Called by:** Manual admin operation.
- **Notes:**
  - Strips the header row if `rows[0]?.[0]?.toLowerCase() === 'zone'` (line 46).
  - Generates a fresh UUID for `rsvps.id` but preserves the original Sheet/Power Apps ID from column E as `sheet_row_id` (lines 90–99) — that is the value later exposed as `powerapps_id`.
...
- **Supabase tables:** `manager_sessions` (select), `managers` (select), `vip_passes` (select / insert / update / delete), `events` (select for `max_passes` + denormalized name/date), `audit_log` (via `logAudit`).
- **External APIs:** None.
...
- **Notes:**
  ...
  - Successful create/delete/edit each fire `logAudit` (`pass.create` / `pass.delete` / `pass.edit`, lines 176, 218, 237, 246, 296).
...
- **Supabase tables:** `manager_sessions` (insert on login, delete on logout, select on verify), `managers` (select for credential check + fallback embed on verify), `audit_log` (via `logAudit`).
- **External APIs:** None.
...
- **Notes:**
  ...
  - `/verify` embeds the manager via `managers(username, permissions)` join. If the embed comes back null (manager was deleted but session row leaked because of no FK cascade), it falls back to a separate `managers` select keyed on `session.manager_id`.
  - Audit hooks (the one place with no pre-existing session, so the actor is passed explicitly): logs `login` on success (superadmin or a specific manager, lines 42 & 72) AND on every failure with a `reason` (`bad_password` / `unknown_user`, lines 47, 59, 65), plus `logout` on sign-out (line 86).
  - The `404 "Not found"` at the bottom is returned for any unrecognized path under `/.netlify/functions/manager-auth`.
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `ADMIN_PASSWORD`.
- **Supabase tables:** `manager_sessions` (select), `invite_lookup_attempts` (select), `invite_ip_whitelist` (select / upsert with `onConflict: 'ip'` / delete), `audit_log` (via `logAudit`).
- **External APIs:** None.
- **Called by:** `public/admin/index.html` Settings → Invite Lookup Security card.
- **Notes:** `added_by` is hard-coded to `'superadmin'` on upsert regardless of which superadmin auth path was used. Whitelist add/remove each fire `logAudit` (`ip.whitelist` / `ip.unwhitelist`, lines 106 & 120).
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY` (with `SUPABASE_ANON_KEY` fallback), `ADMIN_PASSWORD`.
- **Supabase tables:** `manager_sessions` (select for token auth + bulk delete on DELETE), `managers` (insert / update / delete), `audit_log` (via `logAudit`).
- **External APIs:** None.
- **Called by:** `public/admin/index.html` Managers tab.
- **Notes:**
  - bcrypt cost is 10 (`bcrypt.hash(password, 10)` at lines 50 and 70).
  - DELETE does a manual cascade: pre-deletes all `manager_sessions` rows for that `manager_id` BEFORE deleting the `managers` row — there is no DB-level `ON DELETE CASCADE`, so missing this step would leave orphaned sessions that `manager-auth.js` `/verify` would treat as still valid (until expiry).
  - Create/update/delete each fire `logAudit` (`manager.create` / `manager.update` / `manager.delete`, lines 62, 84, 95).
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY` (with `SUPABASE_ANON_KEY` fallback), `ADMIN_PASSWORD`.
- **Supabase tables:** `manager_sessions` (select), `events` (select / insert / update / delete), `audit_log` (via `logAudit`).
- **External APIs:** None.
- **Called by:** `public/admin/index.html` Events tab; `admin-passes.js` indirectly reads the same table via Supabase FK embed.
- **Notes:** The `isSuperadmin` check at line 16 is guarded by `!!process.env.ADMIN_PASSWORD && !!adminPassword` so an empty env var + empty header can't both equal `''` and pass. Create/update/delete each fire `logAudit` (`event.create` / `event.update` / `event.delete`, lines 58, 72, 81).
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`.
- **Supabase tables:** `manager_sessions` (select), `zone_events` (select / upsert), `audit_log` (via `logAudit`).
- **External APIs:** None.
- **Called by:** `public/admin/index.html` (`UPDATE_EVENT_NAME_API`).
- **Notes:**
  - The upsert writes `{ zone, event_name, flyer_hash: null, updated_at: new Date().toISOString() }` (line 89). Setting `flyer_hash = null` is deliberate: it clears the override's anchor so that the NEXT deploy re-anchors this manual rename to the current flyer's hash (see `scripts/automate.js` `resolveEventName()` and the `flyer_hash` override-anchoring mechanism). Without this, a rename would either linger onto a later, different flyer or get stomped immediately.
  - The response headers explicitly include `Access-Control-Allow-Headers: 'x-manager-token, Content-Type'` (line 16), which most other functions omit and rely on Netlify's default preflight permissiveness.
  - A successful rename fires `logAudit` (`event.rename`, line 97).
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`.
- **Supabase tables:** `manager_sessions` (select), `managers` (select), `builder_sessions` (insert), `audit_log` (via `logAudit`).
- **External APIs:** None.
...
- **Notes:** Superadmin defaults: `allowedZones = ['scranton','mountain-top','moosic','bloomsburg','satsang-sabha']`, `allowAdvanced = true` (lines 24–25). Manager defaults come from `manager.permissions.flyer_zones` and `manager.permissions.flyer_builder_advanced`. The row is initialized with `used: false` and `expires_at = now + 30 min`. Minting a session fires `logAudit` (`builder.open`, line 63).
...
- **Notes:** Despite the `used` column existing in `builder_sessions`, this function does NOT flip it on success — the session is reusable for the full 30 minutes, not single-shot. Read-only — no `logAudit` call.
...
- **Supabase tables:** `builder_sessions` (select), `managers` (select for `username`), `flyer_reviews` (insert / delete on rollback), `audit_log` (via `logAudit`). Supabase Storage bucket `flyer-reviews` (upload, `createSignedUrl`, `remove`).
- **External APIs:** Telegram Bot API — `sendPhoto` with a manually-constructed `multipart/form-data` `Uint8Array` body, with an `inline_keyboard` carrying `callback_data: 'review:approve:<reviewId>' | 'review:reject:<reviewId>'`.
- **Called by:** `public/flyer-builder/index.html` when the manager clicks "Send for Review". (Note: the on-screen "PREVIEW" watermark is a draft-only DOM overlay — the bytes uploaded here come from a separate offscreen canvas and carry NO watermark; see the Flyer Builder Front-End section.)
- **Notes:**
  ...
  - The `rejectUrl` is computed up-front from `eventData` so the rejection flow can prefill the builder with the original values; it's stored INSIDE `event_data.rejectUrl` on the review row.
  - Rollback (`rollbackReview`) removes both storage objects and deletes the `flyer_reviews` row. Triggered on any Telegram prep/send failure after the DB insert.
  - OG upload failure is non-fatal — `ogStoragePath` falls back to `null` and the flow continues with just the main flyer.
  - A successful submission fires `logAudit` (`flyer.submit_for_review`, line 202).
...
- **Request:**
  - POST: `{ zone, imageBase64 }`. `imageBase64` is the raw base64 content (no data URI prefix); the admin portal compresses the image client-side (canvas, ≤ 2200px) before POST to stay under Netlify's ~6MB function limit.
  - DELETE: `{ zone }`.
  - `zone` ∈ `['scranton','mountain-top','moosic','bloomsburg','satsang-sabha','mandir-1'..'mandir-5']` (line 47).
- **Response:** `200 { ok: true }`. `400 "Invalid zone"` / `"Missing imageBase64"`. `401 "Unauthorized"` / `"Session expired"`. `403 "No permission to upload flyers"` / `"No permission to remove flyers"`. `404 "No flyer found for <zone>"` on DELETE if the flyer doesn't exist. `413 "Image too large..."` when the base64 payload is pathologically large (should never fire given client-side compression). `500` GitHub error.
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `GITHUB_PAT`.
- **Supabase tables:** `manager_sessions` (select), `managers` (select), `audit_log` (via `logAudit`).
...
- **Notes:**
  - The entire handler is wrapped in a top-level `try/catch` (lines 10–15) that returns a JSON `500` — this closes the "empty body → generic 502" class of failure (a bodyless request, a throw before an inner try, a transient GitHub fetch rejection all now surface as clean JSON).
  - A dedicated `413` guard (line 192) rejects an oversized base64 payload with a clear JSON error instead of letting it hit the raw function limit.
  - The commit messages are `"Upload flyer for <zone> via admin portal"`, `"Remove flyer for <zone> via admin portal"`, `"Remove stale og.jpg for <zone> after flyer upload"`, and `"Remove og.jpg for <zone> via admin portal"`.
  - `deleteOgIfExists` silently no-ops on 404 — `og.jpg` may or may not exist.
  - Every commit triggers the `rsvp-automation.yml` workflow which re-OCRs and re-deploys.
  - Upload/remove (single and bulk) each fire `logAudit` (`flyer.upload` / `flyer.remove` / `flyer.remove_bulk`, lines 141, 224, 263).
...
- **Notes:** The trick is at line 59 — `force_all: zoneOverride ? 'false' : 'true'`. A `zone_override` value implies "only that zone", so `force_all` flips off; an absent value means "everything", so `force_all` is on. A successful dispatch fires `logAudit` (`flyer.refresh`, line 72).
...
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `GITHUB_PAT`.
- **Supabase tables:** `manager_sessions` (select), `audit_log` (via `logAudit`).
...
- **Notes:** Explicitly sets `Access-Control-Allow-Headers: 'x-manager-token, Content-Type'` on every response. The 502 is intentional — distinguishes "GitHub is broken" from "you sent us bad input" (401/403). A successful dispatch fires `logAudit` (`site.redeploy`, line 55).
...
- **Notes (the dense ones):**
  ...
  - `SUFFIX_MAP` covers `/getflyer<suffix>` parsing. Plain `mandir` maps to `mandir-1`, but bare `/getflyer` in the mandir group sends ALL `MANDIR_ZONES` in sequence. `satsang` → `satsang-sabha`.
  - Approve flow atomic claim: `UPDATE flyer_reviews SET status='processing' WHERE id=? AND status='pending' RETURNING *` — only ONE caller can flip pending → processing. Webhook redelivery or admin double-click is silently no-op'd (`if (claimErr || !review) return 200`).
  - Approve flow GitHub work: lists `flyers/<zone>/` and deletes every existing `.jpg/.jpeg/.png` file (with `sha`), THEN PUTs `flyer.jpg` AND `og.jpg`. The `og` content falls back to the main flyer bytes if no OG preview was uploaded. Zone has `-santos` stripped before pathing.
  - Approve flow rollback: on any error in the GitHub block, rolls `processing → pending` BUT guarded with `.eq('id', reviewId).eq('status', 'processing')` so a concurrent reject doesn't get clobbered.
  - Reject flow: no atomic claim, simpler — `status = 'rejected'`, remove storage objects, edit the photo caption, then send a SEPARATE follow-up message with the prebuilt `event_data.rejectUrl`. The link is sent as plain text in its own message specifically to avoid Markdown escaping.
  - Audit hooks: because the callback carries no portal auth, the actor (the Telegram user) is passed to `logAudit` explicitly. Approve fires `flyer.approve` (line 555) and reject fires `flyer.reject` (line 609), each with `details: { via: 'telegram', reviewId }` and `actorId: 'tg:' + <telegram user id>`.
  ...
  - `/summary` dispatches `rsvp-summary.yml` with `target_zone` derived from `ZONE_CHAT_MAP[chatId]` when bare, or from a `summarySuffixMap` when suffixed; `trigger_chat_id: chatId` is what `scripts/summary.js` uses to reply into the calling chat instead of broadcasting.
  - `/refreshall` is admin-chat only — dispatches `rsvp-automation.yml` with `inputs.force_all: 'true'`. Same workflow that `refresh-flyers.js` calls, just with a fixed input.
  - The `sendPhotoBuffer` helper downloads a flyer URL from `screvents.com/<zone>/flyer.jpg` and re-uploads it to Telegram as a `multipart/form-data` body assembled manually with a `Uint8Array` — no `FormData` polyfill needed in the Netlify runtime.

---

### Audit Logging

Every state-changing manager action, every login/logout, and the Telegram-driven flyer approve/reject now leave a durable trail in the `audit_log` Supabase table. The write path is the shared, fire-and-forget `_audit.js` helper (a library module, not an endpoint); the read path is the superadmin-only `superadmin-audit-log.js` endpoint that backs the admin portal's "Audit Log" tab.

#### _audit (shared helper)

- **File:** `netlify/functions/_audit.js`
- **Purpose:** Shared audit-logging helper. Records who/what/when for manager logins and every state-changing manager action into the `audit_log` table. Not a Netlify endpoint — it has no `exports.handler`; it exports `logAudit` / `getClientIp` and is `require('./_audit')`d by the handlers that mutate state.
- **Exports:** `logAudit(supabase, event, { action, target?, details?, success?, actorName?, actorId?, isSuperadmin? })`, `getClientIp(event)`.
- **HTTP method:** N/A (library module).
- **Env vars:** reads `ADMIN_PASSWORD` (to recognize a superadmin credential); otherwise uses the caller-provided Supabase client.
- **Supabase tables:** `audit_log` (insert), plus best-effort background lookups on `manager_sessions` / `managers` / `builder_sessions` to resolve the actor's name from the request credential.
- **External APIs:** None.
- **Called by:** All 14 state-changing functions — `get-rsvps`, `manage-rsvp`, `admin-passes`, `admin-rate-limit`, `superadmin-events`, `superadmin-managers`, `update-event-name`, `manager-auth`, `create-builder-session`, `review-flyer`, `manage-flyers`, `refresh-flyers`, `trigger-redeploy`, and `telegram-webhook`.
- **Notes:**
  - Deliberately FIRE-AND-FORGET: it never `await`s and never throws. The whole body is wrapped in a `try/catch`, and the insert ends `.then(() => {}, () => {})` (line 40) so every error is swallowed. A logging failure — including the `audit_log` table not existing yet — can never break or slow the audited action.
  - Actor derivation: call sites do NOT thread identity. The helper derives the actor in the background from the SAME credential the request already carries — `x-admin-password` (→ `admin`, superadmin, line 57), `x-manager-token` (→ `manager_sessions.manager_id`; NULL ⇒ superadmin, lines 60–64), or `x-builder-session` (→ `builder_sessions.manager_id` / `is_superadmin`, lines 65–69).
  - Two call sites cannot rely on a session and pass an explicit actor: `manager-auth.js` login (no session row exists yet) and `telegram-webhook.js` approve/reject (the callback carries no portal auth) — they pass `actorName` / `actorId` / `isSuperadmin` directly (handled first, lines 51–54).
  - A missing username is backfilled from the manager id (a separate `managers.username` lookup) so the log stays readable even after that manager is later deleted (lines 45–48).
  - Each row records `actor_id`, `actor_name` (defaults to `'admin'` for superadmin, else `'unknown'`), `is_superadmin`, `action`, `target`, `details` (jsonb), `success` (defaults `true`), `ip`, and `user_agent` (UA sliced to 400 chars). `getClientIp` falls through `x-nf-client-connection-ip` → `x-forwarded-for[0]` → `client-ip` → `'unknown'`.

#### superadmin-audit-log

- **File:** `netlify/functions/superadmin-audit-log.js`
- **Purpose:** Superadmin-only reader for the `audit_log` table — powers the admin portal's "Audit Log" tab.
- **HTTP method:** `GET` only (plus `OPTIONS`; anything else → `405`).
- **Auth:** Superadmin only — `x-admin-password === ADMIN_PASSWORD` OR `x-manager-token` mapping to a session with `manager_id === null` (lines 17–33).
- **Request:** Optional `?limit=` (clamped to 1–500, default 200).
- **Response:** `200` array of `{ created_at, actor_name, is_superadmin, action, target, details, success, ip, user_agent }` ordered `created_at DESC`. `401 "Superadmin access required"`. `405` non-GET. `500` on DB/read error.
- **Env vars:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `ADMIN_PASSWORD`.
- **Supabase tables:** `manager_sessions` (select for token auth), `audit_log` (select).
- **External APIs:** None.
- **Called by:** `public/admin/index.html` Audit Log tab (superadmin-only). The tab renders who / what / when plus a parsed DEVICE label (e.g. "iPhone · Safari") derived client-side from the row's `user_agent`, with the raw IP shown on hover.
- **Notes:**
  - Read-only — nothing here mutates state (and no `logAudit` call). The whole handler is wrapped in a single `try/catch`.
  - `audit_log` has RLS enabled and is service-role only, so this endpoint (service key) is the only supported read path; the table itself is created by migration `scripts/migrations/2026-08-28-add-audit-log.sql` (run once in Supabase — see the Recreate Steps and Supabase Schema sections).

---

## 6. Background Scripts Reference
The repo ships four long-form Node scripts under `/Users/pritpatel/Desktop/SCREvents/scripts/` that run inside GitHub Actions and do everything that doesn't fit in a per-request Netlify function: flyer OCR + zone-page generation, RSVP archival, and RSVP digests. Every script declares its dependencies in `scripts/package.json` (`@anthropic-ai/sdk`, `@supabase/supabase-js`, `googleapis`, `sharp`, `axios`, `xlsx`, `adm-zip`) and is invoked from a workflow under `.github/workflows/` after a `cd scripts && npm install` step. See [GitHub Workflows] for the cron schedules and dispatch inputs, [Supabase Schema] for the tables these scripts read and write, and [Env Vars Master] for the full list of secrets each one needs.

### `scripts/automate.js`

The end-to-end flyer-to-Netlify pipeline. This is by far the largest script in the repo and is the only one that ever writes to the `dist/` build output. It walks every zone folder under `/flyers/<zone>/`, OCRs each flyer through Claude vision, resolves the canonical event name against Supabase (anchored to a hash of the flyer bytes), renders per-zone landing pages, builds Open Graph share images, syncs everything into `dist/`, commits and pushes `dist/` back to `main`, and updates `deadlines.json` at the repo root for every other script to consume.
...
- **Env vars consumed** (read at runtime via `process.env`):
  ...
  - (No Google Sheet credentials — this script does not touch the retired Sheet. The workflow still passes `GOOGLE_SERVICE_ACCOUNT_JSON` for historical reasons, but `automate.js` never reads it.)
- **Supabase tables touched**:
  - `zone_events` — `SELECT event_name, flyer_hash WHERE zone=?`, `UPSERT {zone, event_name, flyer_hash, updated_at}` when the row is missing, `UPDATE event_name=?, flyer_hash=?` when the stored value is empty or the flyer changed, `UPDATE flyer_hash=?` to re-anchor a legacy (null-hash) row, and bulk `UPDATE event_name='' WHERE zone IN (...)` for any zone whose flyer was removed. Also a preliminary `SELECT zone, flyer_hash` to pre-load stored hashes.
...
- **Internal helpers worth naming**:
  ...
  - `resolveEventName(supabase, zone, ocrName, flyerHash)` — implements the DB-vs-OCR precedence rule **anchored to the flyer**. It reads `{event_name, flyer_hash}` from `zone_events`. A stored non-empty name (an admin override) wins **only while its `flyer_hash` still matches the SHA-1 of the current flyer's bytes**; if the flyer changed (hash differs) the stored override is discarded and the fresh OCR name is reseeded, so an admin's rename never lingers onto the next event uploaded to that zone. A missing/empty row seeds the OCR name anchored to the current hash; a legacy row with `flyer_hash = null` keeps its name but is re-anchored to the current flyer on this run (preserving intentional overrides on the first hash-aware deploy). If the `flyer_hash` column doesn't exist yet (migration not run), it degrades gracefully to the pre-hash behavior (stored name wins) instead of erroring.
...
- **Step-by-step flow** (top-level `main()`):
  ...
  3. Walk `flyers/<zone>/` for the fixed zone list `['satsang-sabha', 'mountain-top', 'scranton', 'moosic', 'bloomsburg', 'mandir-1'..'mandir-5']`. Collect the first non-`og.jpg` `.png|.jpg|.jpeg` per zone, and compute `flyerHash = SHA-1` of that flyer's bytes. Build `zonesWithFlyer` set as a side-effect.
  ...
  5. Load `deadlines.json` (repo root) as `cachedDeadlines`. Missing/corrupt file is treated as `{}`. Pre-load `storedFlyerHashes` via `SELECT zone, flyer_hash FROM zone_events` (empty if the column doesn't exist yet).
  6. For each flyer compute `flyerChanged = storedHash !== null && storedHash !== flyerHash`, then `skipOcr = !FORCE_ALL && !isChanged && hasCached && zone !== ZONE_OVERRIDE && !flyerChanged`. If `skipOcr`, reuse the cached `{eventName, date, time, location, deadline}` blob; otherwise call `extractEventInfo()` to OCR via Claude. `flyerChanged` forces a fresh OCR even when the git diff didn't flag the flyer (e.g. a collapsed deploy run) so the reseeded name comes from the new flyer, not stale cache.
  7. `resolveEventName(supabase, zone, eventInfo.eventName, flyerHash)` — the stored admin override wins only while its `flyer_hash` matches the current flyer's SHA-1; a changed flyer resets the name to the fresh OCR value, otherwise the OCR name is seeded (anchored to the hash) into `zone_events`.
...
- **`deployAllToNetlify()` flow** (the publish half of the pipeline):
  ...
  7. If there is something staged, `git commit -m "deploy: update dist"`, then try `git push origin HEAD:main` up to **5 times**, doing `git pull --rebase --autostash -X theirs origin main` between attempts. Because `dist/` is 100% generated and this run rebuilds *every* zone from the current repo state, the just-built `dist/` is authoritative — `-X theirs` auto-resolves any `dist/` conflict from a concurrent deploy in our favor (during a rebase, "theirs" is the commit being replayed, i.e. our own deploy commit) instead of dying on an unresolvable merge. `--autostash` keeps the still-unstaged `deadlines.json` write from blocking the rebase. After 5 failures the script throws so the workflow turns red.
- **Outputs**:
...
Event-night archive job. For every zone whose `eventDate` (from `deadlines.json`) is strictly before today in `America/New_York`, the script copies the active `rsvps` rows into `rsvps_archive` (preserving `id`, adding `archived_at` + `event_date`), then deletes them from `rsvps`, then **best-effort** deletes the same rows from the legacy Google Sheet, then posts a Telegram summary. The Google Sheet is retired as the live RSVP store (see [Supabase Schema] — `submit-rsvp.js` writes Supabase only), so the Sheet-delete step is a legacy leftover that no-ops harmlessly if the Sheet credentials are absent. Re-runs are idempotent (`upsert` on `id`).
...
- **Env vars consumed**:
  ...
  - `GOOGLE_SHEET_ID` — legacy Sheet to clean (best-effort only).
  - `GOOG_SA_JSON` (preferred) or `GOOGLE_SERVICE_ACCOUNT_JSON` (fallback) — Google service-account credentials JSON, used only by the legacy best-effort Sheet cleanup. Optional — the script survives their absence.
...
- **External APIs called**:
  - Google Sheets API v4 (legacy best-effort) — `spreadsheets.values.get` on `responses!A:E`, `spreadsheets.get` to look up `sheetId`, `spreadsheets.batchUpdate` with `deleteDimension` requests sorted descending so row indexes don't shift mid-batch.
...
- **Step-by-step flow**:
  ...
  8. **Step 2 — delete from `rsvps`**: `DELETE WHERE zone IN (zoneList)`. On failure send `RSVP archive partial` Telegram and throw — the archive insert already succeeded so data is safe.
  9. **Step 3 — Sheet best-effort (legacy)**: `deleteSheetRowsForZones`. Any throw is logged but does NOT fail the script; Supabase is the sole source of truth. Because the Sheet is retired, this step is a leftover that harmlessly no-ops if the Sheet is gone or credentials are missing.
...
- **Outputs**:
  ...
  - Google Sheets mutation (legacy best-effort): matching rows removed from the `responses` tab if the Sheet still exists.
...
Sends a "Final RSVP List" Telegram message for every zone whose `eventDate` equals today in `America/New_York`. Pulls RSVPs from the Google Sheet via the public CSV export endpoint (no service-account auth needed) and posts a name-and-guest list with totals to the admin Telegram chat. Designed as the last reminder of the headcount on event day before `cleanup-past-rsvps.js` archives them the next morning. **Note:** the Google Sheet is retired as the live RSVP store (`submit-rsvp.js` writes Supabase only), so this script is a legacy tool still pointed at the Sheet CSV; the every-3-days digest (`summary.js`) reads Supabase directly instead.
...
- **Env vars consumed**:
  ...
  - `GOOGLE_SHEET_ID` — legacy CSV export source.
...
All scheduled automation lives under `.github/workflows/` and runs on `ubuntu-latest`. The five workflows handle flyer OCR + deploy, headcount summaries, Supabase keep-alive, and post-event archival. Every workflow can also be manually triggered via `workflow_dispatch` from the GitHub Actions UI.
...
| `cleanup-past-rsvps.yml` | `0 8 * * *` and `0 9 * * *` | 4 AM ET (year-round via dual cron) | Moves past-event rows from `rsvps` to `rsvps_archive` (and best-effort clears the retired Sheet). |
...
**Build filtering (Netlify, not GitHub Actions).** Netlify has its own build-skip filter that is separate from these workflows: `netlify.toml`'s `ignore` command —
```
git diff --quiet "$CACHED_COMMIT_REF" "$COMMIT_REF" -- ':(exclude)flyers' ':(exclude)scripts' ':(exclude).github' ':(exclude)*.md'
```
— tells Netlify to **skip** a build when a commit changes nothing that affects the deploy, i.e. only `flyers/` (source images the GitHub Action processes, never served directly), `scripts/` (automation that runs on GitHub Actions), `.github/` (workflows), or `*.md` docs. Any change to `dist/`, `netlify/` (functions), `public/`, `build.sh`, or `netlify.toml` still builds. It fails **safe**: exit 0 skips, any other exit builds, and an empty/invalid `$CACHED_COMMIT_REF` errors out (exit 128) → builds. This halves the double-commit churn from the flyer pipeline — a flyer-upload commit is skipped, and the `deploy: update dist` commit it triggers still builds. See Netlify Config & Build for the full `netlify.toml`.
...
- **Secrets used:** `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `GOOGLE_SHEET_ID`, `GOOG_SA_JSON`, `GOOGLE_SERVICE_ACCOUNT_JSON`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`. (The three Google/Sheet secrets are only consumed by the legacy best-effort Sheet-cleanup step and are optional.)
- **Script:** `scripts/cleanup-past-rsvps.js`.

**Purpose.** Runs the morning AFTER an event (4 AM local) so attendees can still be checked in at the door on event day itself. For every zone in `deadlines.json` whose `eventDate` is strictly before today (America/New_York), the script upserts `rsvps_archive` (keyed on `id`), deletes the matching rows from `rsvps`, best-effort deletes the matching rows from the retired Google Sheet, then posts an archive summary to `TELEGRAM_CHAT_ID`. The dual-cron pattern means EDT/EST switchovers are handled without DST math in the workflow.
...
**Purpose.** Sends the end-of-day "final RSVP" Telegram digest. For each zone whose `eventDate` equals today in America/New_York, the script downloads the public CSV export of the (legacy) Google Sheet, filters by zone, formats a name + guest-count list with totals, and posts one HTML message per active zone to `TELEGRAM_CHAT_ID`. It's deliberately the last summary push of the day so organizers see the most current headcount before going to bed. Drives strictly off the CSV — no Supabase credentials in this workflow. (Legacy: the Sheet is retired as the live RSVP store; the every-3-days `rsvp-summary.yml` reads Supabase instead.)
...
- **Steps:**
  ...
  5. `Commit deadlines.json` — configures `github-actions` git identity, stages `deadlines.json`, no-ops if there's no diff, otherwise commits `Auto-update deadlines.json [skip ci]` and pushes to `origin main`. Retries up to **5 times** with `git pull --rebase --autostash -X theirs origin main` between attempts to survive a remote that moved during the run — `deadlines.json` is regenerated from the current flyers every run, so our version is authoritative and `-X theirs` auto-resolves any conflict in our favor instead of dying on a merge. The `[skip ci]` tag stops the resulting commit from re-triggering this workflow.
- **Secrets used:** `ANTHROPIC_API_KEY`, `GOOGLE_SERVICE_ACCOUNT_JSON`, `NETLIFY_AUTH_TOKEN`, `NETLIFY_SITE_ID`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`. `GITHUB_REPOSITORY` is also passed (filled by `${{ github.repository }}`). (`GOOGLE_SERVICE_ACCOUNT_JSON` is passed for historical reasons but `automate.js` no longer reads it — the Sheet is retired.)
...
## 8. Supabase Schema
The project uses a single Supabase Postgres project (URL + service-role key live in `SUPABASE_URL` / `SUPABASE_SERVICE_KEY` — see Env Vars Master) as the system of record for RSVPs, VIP passes, manager accounts, sessions, flyer reviews, IP rate-limit state, Telegram bot wizard state, and an audit log. There is a `scripts/migrations/` folder with incremental `.sql` patches (two so far — `2026-08-28-add-audit-log.sql` and `2026-08-28-add-flyer-hash.sql`), but **no full baseline schema definition in the repo** — no `supabase/` folder, no consolidated `schema.sql`. The base tables, foreign keys, and indexes live only in the Supabase project that the deployed env vars point at; the two migration files are canonical only for the specific additions they make (the `audit_log` table and the `zone_events.flyer_hash` column). The shape below is reverse-engineered from the queries the Netlify functions and GitHub Actions scripts actually run, folded together with those migrations. The Schema Bootstrap subsection at the end is a paste-into-the-Supabase-SQL-editor recreation of that inferred shape.
...
The GitHub Actions scripts (`scripts/automate.js`, `scripts/cleanup-past-rsvps.js`, `scripts/summary.js`) construct the same client with the service key from GitHub repo secrets.
...
**Purpose.** Live (upcoming-event) RSVP submissions, populated from the public zone landing pages. **The sole source of truth for headcounts** — Supabase only. The legacy Google Sheet mirror was retired: `submit-rsvp.js` writes only this table, and `get-rsvps.js` reads only this table (no Sheet merge/dual-write).
...
| `sheet_row_id` | `text` | **Legacy-named cross-key** — it is simply the row's own UUID (`submit-rsvp.js` sets `sheet_row_id = id`). The name dates back to the Google Sheet era but there is no Sheet anymore; it survives as the stable id the admin API keys `UPDATE`/`DELETE` on. The admin API (`get-rsvps.js`) exposes this value under the JSON field name `powerapps_id` in both responses and edit/delete requests — that name is likewise legacy and kept for compatibility (no live PowerApps integration exists). |

**Read by.** `netlify/functions/get-rsvps.js` (`SELECT zone, name, guests, submitted_at, sheet_row_id ORDER BY submitted_at DESC`, then remapped to `powerapps_id` in the response); `netlify/functions/backfill-rsvps.js` (`SELECT sheet_row_id` for dedup); `scripts/cleanup-past-rsvps.js` (`SELECT id, zone, name, guests, event_name, submitted_at, sheet_row_id WHERE zone IN (...)`); `scripts/summary.js` (full table scan, filtered in JS).

**Written by.** `netlify/functions/submit-rsvp.js` (`INSERT`, with `sheet_row_id = id`); `netlify/functions/get-rsvps.js` (`UPDATE guests WHERE sheet_row_id` / `DELETE WHERE sheet_row_id`, keyed on the `powerapps_id` from the request); `netlify/functions/restore-rsvps.js` (`UPSERT onConflict: 'id'`); `scripts/cleanup-past-rsvps.js` (`DELETE WHERE zone IN (...)`).
...
| `sheet_row_id` | `text` | Same legacy-named UUID cross-key as in `rsvps`. |
...
**Read by.** `netlify/functions/manager-auth.js` (login by username + verify embedded join `managers(username, permissions)`); `netlify/functions/superadmin-managers.js` (list); `netlify/functions/get-rsvps.js`, `get-archived-rsvps.js`, `manage-flyers.js`, `refresh-flyers.js`, `create-builder-session.js`, `admin-passes.js`, `review-flyer.js` (`SELECT permissions` or `SELECT username, permissions` by id during per-request authz); `netlify/functions/_audit.js` (`SELECT username WHERE id = ?` to backfill a readable actor name for the audit log).
...
**Read by.** Every gated endpoint: `manager-auth.js` (`/verify` with embedded `managers(username, permissions)`), `admin-rate-limit.js`, `admin-passes.js`, `get-rsvps.js`, `get-archived-rsvps.js`, `manage-flyers.js`, `manage-rsvp.js`, `refresh-flyers.js`, `superadmin-events.js`, `superadmin-managers.js`, `superadmin-audit-log.js`, `trigger-redeploy.js`, `update-event-name.js`, `create-builder-session.js`; also `netlify/functions/_audit.js` (`SELECT manager_id WHERE token = ?` to derive the actor for an audited action).
...
**Read by.** `netlify/functions/telegram-webhook.js` (`SELECT * WHERE id = ?` on reject); `netlify/functions/_audit.js` never reads it, but the approve/reject actions on this table are audit-logged (see `audit_log`).
...
**Read by.** `netlify/functions/verify-builder-session.js` (`SELECT * WHERE id = ?`); `netlify/functions/review-flyer.js` (`SELECT * WHERE id = ?`); `netlify/functions/_audit.js` (`SELECT manager_id, is_superadmin WHERE id = ?` to derive the actor when a request authenticates via `x-builder-session`).
...
**Purpose.** Per-zone canonical event name (with a last-updated timestamp and a per-flyer anchor hash). `scripts/automate.js` consults this first when rendering a zone landing page and only falls back to OCR'd text from the flyer if no row exists for the zone; if OCR seeds a name, the script `UPSERT`s it so admins can later edit it from the admin portal without losing it on the next OCR run. An admin's override is now tied to the *specific flyer* it was set for via `flyer_hash` (a SHA-1 of the flyer bytes): when a new flyer replaces the old one, its hash no longer matches and `resolveEventName()` resets the name to the fresh OCR value, so a rename never lingers onto the next event. `netlify/functions/update-event-name.js` is the superadmin UI for editing the name.
...
| `flyer_hash` | `text` NULL | SHA-1 of the flyer bytes the current `event_name` is anchored to (added by `scripts/migrations/2026-08-28-add-flyer-hash.sql`). `automate.js` writes it whenever it seeds/updates the name; `resolveEventName()` resets the name to fresh OCR when the current flyer's hash differs. `NULL` for legacy rows (re-anchored on the next deploy) and after an admin rename — `update-event-name.js` sets `flyer_hash = null` on rename so the next deploy re-anchors the new name to the current flyer. |
| `updated_at` | `timestamptz` | Set explicitly on upsert. |

**Read by.** `scripts/automate.js` (`SELECT event_name, flyer_hash WHERE zone = ?`, plus a preliminary `SELECT zone, flyer_hash` to pre-load stored hashes); `netlify/functions/update-event-name.js` (`SELECT zone, event_name, updated_at ORDER BY zone`).

**Written by.** `scripts/automate.js` (`UPSERT { zone, event_name, flyer_hash, updated_at }`, `UPDATE event_name/flyer_hash WHERE zone = ?`, re-anchor `UPDATE flyer_hash WHERE zone = ?`, bulk `UPDATE event_name = '' WHERE zone IN (...)`); `netlify/functions/update-event-name.js` (`UPSERT { zone, event_name, flyer_hash: null, updated_at }`).
...
**Purpose.** Audit log of every `/invite` name+phone lookup attempt. Drives both the per-IP rate limit (5 fails per 60 minutes) and the admin "recent activity" feed. (Distinct from the general `audit_log` table below, which covers manager actions.)
...
**Purpose.** Who / what / when for manager logins and every state-changing manager action. Written **fire-and-forget** by the shared helper `netlify/functions/_audit.js` and read (superadmin-only) by `netlify/functions/superadmin-audit-log.js`, which backs the admin portal's superadmin-only "Audit Log" tab. Added by `scripts/migrations/2026-08-28-add-audit-log.sql`.
...
| --- | --- | --- |
| `id` | `bigserial` PK | |
| `created_at` | `timestamptz` | `default now()`; `ORDER BY created_at DESC` on read. |
| `actor_id` | `text` NULL | `managers.id` as text for a manager; `'tg:<telegramUserId>'` for a Telegram button press; `null` for superadmin/unknown. |
| `actor_name` | `text` | `default 'unknown'`. `'admin'` for the superadmin, else the manager username (backfilled from `managers` when only an id is known). |
| `is_superadmin` | `boolean` | `default false`. |
| `action` | `text` | e.g. `'login'`, `'logout'`, `'flyer.upload'`, `'flyer.approve'`, `'flyer.reject'`, `'rsvp.delete'`, `'event.rename'`. |
| `target` | `text` NULL | The object acted on (zone, id, username, …). |
| `details` | `jsonb` NULL | Extra structured context (e.g. `{ via: 'telegram', reviewId }`, `{ reason: 'bad_password' }`). |
| `success` | `boolean` | `default true`; `false` for e.g. a failed login. |
| `ip` | `text` NULL | Derived from the request headers (`x-nf-client-connection-ip` → `x-forwarded-for[0]` → `client-ip`). |
| `user_agent` | `text` NULL | Truncated to 400 chars. The admin tab parses this into a device label (e.g. "iPhone · Safari") shown per row, with the raw IP on hover. |

**The helper.** `_audit.js` exports `logAudit(supabase, event, { action, target, details })`. It is deliberately fire-and-forget — it never `await`s and never throws, so a logging failure (including the table not existing yet) can never break or slow the audited action. It derives the actor **from the request's own auth credential** (`x-admin-password` → superadmin, `x-manager-token` → look up `manager_sessions`, `x-builder-session` → look up `builder_sessions`) and records `ip` + `user_agent`. Login is the one exception (no session exists yet), so `manager-auth.js` passes an explicit `actorName` / `isSuperadmin`.

**Hooks.** `manager-auth.js` logs login success/failure (with a `reason` on failure) and logout; roughly a dozen state-changing functions log their action — `admin-passes.js`, `admin-rate-limit.js`, `create-builder-session.js`, `get-rsvps.js` (edit/delete), `manage-flyers.js`, `manage-rsvp.js`, `refresh-flyers.js`, `review-flyer.js`, `superadmin-events.js`, `superadmin-managers.js`, `trigger-redeploy.js`, `update-event-name.js`; and `telegram-webhook.js` logs flyer approve/reject (with the Telegram user as the actor).

**Read by.** `netlify/functions/superadmin-audit-log.js` — a `GET`, superadmin-only reader that `SELECT created_at, actor_name, is_superadmin, action, target, details, success, ip, user_agent ORDER BY created_at DESC LIMIT <=500` (default 200).

**Written by.** `netlify/functions/_audit.js` (`INSERT`, fire-and-forget) from every hook above.

**RLS posture.** RLS is **enabled** on this table with **no policies**, so only the service-role key (the Netlify functions) can read or write it — no anon/public access. This matches the app's overall "service-key only, deny-by-default" posture and is the one table where the migration explicitly turns RLS on.

**Suggested indexes.**
- `(created_at DESC)` — `audit_log_created_at_idx`, the sole ordering used by the reader.
...
The repo has only incremental migrations under `scripts/migrations/` (adding the `audit_log` table and the `zone_events.flyer_hash` column) and no full baseline schema, so this SQL is the closest thing to a canonical full-schema definition. Paste it into the Supabase SQL editor on a fresh project to recreate the inferred shape; it already folds in both migration files. **Some column types are best-guess** — they are flagged with `-- guess:` comments. Adjust to taste based on what you actually want to enforce.
...
create table rsvps (
  ...
  sheet_row_id text not null                  -- legacy-named cross-key; simply equals id (Sheet retired)
);
...
create table rsvps_archive (
  ...
  event_name text,
  submitted_at timestamptz,
  sheet_row_id text,                          -- legacy-named UUID cross-key
...
-- flyer_hash added by scripts/migrations/2026-08-28-add-flyer-hash.sql: it anchors
-- the override to the SHA-1 of the flyer it was set for, so a replaced flyer resets
-- the name to fresh OCR instead of letting a stale rename linger.
create table zone_events (
  zone text primary key,                      -- 'scranton' | 'mountain-top' | 'moosic' | 'bloomsburg' | 'satsang-sabha' | 'mandir-1'..'mandir-5'
  event_name text not null default '',
  flyer_hash text,                            -- SHA-1 of the anchored flyer's bytes; NULL = re-anchor next deploy
...
-- ─── audit log (scripts/migrations/2026-08-28-add-audit-log.sql) ───────────
create table audit_log (
  id            bigserial primary key,
  created_at    timestamptz not null default now(),
  actor_id      text,                             -- managers.id as text; 'tg:<id>' for Telegram; null for superadmin/unknown
...
-- RLS ON with NO policies: only the service-role key (the Netlify functions)
-- can read or write. This is the one table the migration explicitly locks down.
...
**RLS posture across the app.** Only `audit_log` has RLS explicitly enabled (with no policies, i.e. service-role-only) via its migration. Every other table is likewise reached exclusively server-side with `SUPABASE_SERVICE_KEY`, which bypasses RLS. If you want to harden the rest of the database (defense in depth in case the service key ever leaks), enable RLS on every table and add a single deny-all policy — the app will continue to work because the service key still bypasses. Do *not* enable RLS without that deny-all unless you intend to also add the anon-readable policies that none of the current code paths require.
...
All Supabase writes use `SUPABASE_SERVICE_KEY` (see Supabase Schema); the webhook secret is the only perimeter check. Approve/reject presses are also recorded to the `audit_log` table (see below).
...
| `review:approve:<reviewId>` | — (admin chat only) | Runs the flyer-review approval state machine (next section); audit-logged as `flyer.approve`. |
| `review:reject:<reviewId>` | — (admin chat only) | Marks the review rejected, removes Supabase Storage objects, replies with a prefilled rebuild link; audit-logged as `flyer.reject`. |
...
**Auditing.** Both button paths call `logAudit(supabase, event, { actorName: cbActor, actorId: 'tg:'+cbFrom.id, action: 'flyer.approve' | 'flyer.reject', target: review.zone, details: { via: 'telegram', reviewId } })` — fire-and-forget, so the entry lands in `audit_log` with the Telegram user who tapped the button as the actor (the bot path carries no portal auth, hence the explicit actor). See the `audit_log` table in Supabase Schema.
...
The webhook is only the inbound side. Node scripts running in GitHub Actions (and two Netlify functions) push notifications back into the same chats:
...
**Draft-only "PREVIEW" watermark.** While the manager is designing, `public/flyer-builder/index.html` paints a tiled, diagonal **PREVIEW** overlay across the on-screen draft — a sibling `.preview-watermark` `<div>` (a repeating inline-SVG background, white fill + faint black stroke) layered over BOTH the portrait `#flyer-canvas` and the landscape `#og-canvas`. It is a pure DOM overlay and is never drawn into any canvas. Because `sendForReview()` (phase 4) rasterizes from its own fresh **offscreen** canvases and `toDataURL()` reads back only canvas pixels — never the sibling watermark div — the `imageBase64` / `ogBase64` sent to review, and therefore the published `flyer.jpg` / `og.jpg`, carry **no** watermark. The overlay exists only so an in-progress screenshot of the draft can't be mistaken for the finished, approved flyer.
...
- **What happens:**
  1. Validates that date, hour, and address line 1 are filled and that a photo has loaded.
  2. Renders the flyer onto a **fresh, throwaway offscreen `<canvas>`** (`renderFlyer(1500, canvas)` — captured above the deploy cap so it downsamples crisp) and exports `canvas.toDataURL('image/jpeg', 0.92)` → base64 → `imageBase64`.
  3. Renders the OG / social card onto a **second offscreen `<canvas>`** (`renderOG(2400, canvas)` — hi-res so the announcement downsamples rather than upscales) and exports `canvas.toDataURL('image/jpeg', 0.9)` → base64 → `ogBase64`.
  4. POSTs to `/.netlify/functions/review-flyer` with header `x-builder-session: <sessionId>` and JSON body:

```json
{
  "zone": "scranton",
  "imageBase64": "<base64-jpeg, native ~1125px wide>",
...
- **Audit:** on a successful submit the function fires `logAudit(supabase, event, { action: 'flyer.submit_for_review', target: zone, details: { reviewId } })` (see **Auth & Security › Audit logging**).
...
- **`netlify/functions/manage-flyers.js`** — direct upload/delete bypass used from the admin portal's Flyers tab. POST takes `{ zone, imageBase64 }` and PUTs `flyers/<zone>/flyer.jpg` directly (no Telegram review). DELETE removes the same path. Both paths also DELETE `flyers/<zone>/og.jpg` so the next `automate.js` run regenerates a fresh OG image. Auth is `x-manager-token` + `permissions.upload_flyers` / `permissions.remove_flyers` (superadmin bypasses both perm checks). Commit messages are `"Upload flyer for <zone> via admin portal"` and `"Remove flyer for <zone> via admin portal"`; successful uploads/removals also fire `logAudit` (`flyer.upload` / `flyer.remove` / `flyer.remove_bulk`). **Client-side compression + payload guards (important):** before POSTing, the admin portal compresses the chosen image entirely in the browser via `compressFlyer()` in `public/admin/index.html` — it draws the flyer onto a `<canvas>` scaled so its longest edge is ≤ **2200px**, flattens any transparency onto white, and re-encodes to JPEG, stepping `quality` down from 0.92 until the base64 payload is under ~4.5M chars, comfortably inside Netlify's ~6MB function-request limit (a raw phone photo would otherwise blow it and 502 with an empty body, which the client can't JSON-parse). On the server, `manage-flyers.js` wraps its entire handler in a **top-level `try/catch`** so any uncaught throw returns a JSON 500 rather than a bodyless 502, and it rejects an oversized `imageBase64` (> 5.5M chars) with an explicit **HTTP 413** JSON error instead of crashing.
...
- **`netlify/functions/trigger-redeploy.js`** — superadmin-only `workflow_dispatch` on `rsvp-automation.yml` with no inputs, for the "force a fresh deploy" button. Logs `site.redeploy`.
...
These three paths are static asset folders that the builder page loads via `<img>` / `Image()` and `@font-face`, then composites onto a `<canvas>` whose pixels are read back with `canvas.toDataURL()` to a JPEG (phase 4). Two practical reasons the wildcard is required:
...
Manager browser     create-builder-session       Supabase            verify-builder-session   review-flyer        Supabase Storage    flyer_reviews      Telegram Bot API   Admin (Telegram)   telegram-webhook    GitHub API           GH Actions / Netlify
  ...
  3.  │── GET /flyer-builder/templates|preview-templates|fonts/* (CORS *) ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────►│ (static assets)        │
      │ (manager fills form, canvas.toDataURL → imageBase64 + ogBase64)                                                                                                                                                                                                                     │
      │                                                                                                                                                                                                                                                                                          │
...
4. `INSERT` into `rsvps` with `{ id, zone, name, guests, event_name, submitted_at, sheet_row_id: id }`, then returns `{ ok: true, id }`. **Supabase is the sole store** — there is no Google Sheet mirror, append, or dual-write on the live path (the Sheet has been retired). `sheet_row_id` is set to the same UUID as `id`; despite its name it is *not* a Sheet reference — it is a legacy column NAME for the row's own UUID, kept as a stable cross-key that the admin PATCH/DELETE paths still look rows up by.
...
On `200` the page hides the form and shows a thank-you panel. **The RSVP flow does NOT generate a VIP pass** — they are two separate systems. There is no side effect outside the `rsvps` insert: no email, no Google Sheet write (the mirror is retired), and no VIP pass.
...
5. **Legacy, best-effort:** `cleanup-past-rsvps.js` is one of a handful of legacy tools that still reference the retired Google Sheet env vars (`GOOGLE_SHEET_ID` / `GOOG_SA_JSON` / `GOOGLE_SERVICE_ACCOUNT_JSON`). If those are still configured it also deletes the matching rows from the old `responses` sheet via `batchUpdate.deleteDimension` (sorted descending row indices). Supabase is the source of truth; this Sheet touch is optional and never gates the archive.
6. Posts a per-zone HTML summary to `TELEGRAM_CHAT_ID` (admin group).

To **undo** an early/accidental archive, `netlify/functions/restore-rsvps.js` (superadmin password only — `x-admin-password`) walks `rsvps_archive`, optionally filtered by `{ zone, event_date }`, `UPSERT`s back into `rsvps` on conflict `id`, then deletes the restored rows from the archive. It also — legacy, best-effort — deduplicates against the retired Sheet's column E and re-appends the missing rows *if* the Sheet env vars are still present; `restore-rsvps.js` (along with `backfill-rsvps.js`) is another of the legacy tools that still reference those vars. The live RSVP path never touches the Sheet. Response shape:
...
- `GET /.netlify/functions/get-rsvps` → returns Supabase `rsvps` as the **sole** source of truth (the Google Sheet mirror has been retired; there is no Sheet merge). Response shape: `[{ zone, name, guests, submitted, powerapps_id }, …]` newest-first, where `powerapps_id` is just the row's `sheet_row_id` (its UUID) surfaced under a legacy field name the admin UI still uses as its row key. There is no live PowerApps integration either.
- `PATCH /.netlify/functions/get-rsvps` with `{ powerapps_id, guests }` → Supabase-only update, matched by `sheet_row_id` (= `powerapps_id`). Logs `rsvp.edit`.
- `DELETE /.netlify/functions/get-rsvps` with `{ powerapps_id }` → Supabase-only delete, matched by `sheet_row_id` (= `powerapps_id`). Logs `rsvp.delete`.
- `GET /.netlify/functions/get-archived-rsvps?zone=&event_date=&q=` → reads `rsvps_archive` plus filter facets `{ zones, event_dates }`.
- `POST /.netlify/functions/manage-rsvp` with `{ zone, enabled }` → superadmin-only `rsvp_settings` upsert; the `global` meta-zone short-circuits every zone. Logs `rsvp.toggle`.
...
| `scripts/final-summary.js` | `.github/workflows/final-summary.yml` | cron `0 1 * * *` (9 PM EDT the evening before) | Legacy Google Sheet CSV export | `TELEGRAM_CHAT_ID` admin group, only for zones whose `eventDate` equals today (NY) |

`summary.js` resolves the destination chat from a slug→env-var map (`TELEGRAM_CHAT_ID_SCRANTON`, etc., with `TELEGRAM_CHAT_ID_MANDIR` covering `satsang-sabha` + `mandir-1..5`). The `target_zone` dispatch input accepts `scranton | mountain-top | moosic | bloomsburg | mandir | all`; `mandir` expands to the six-zone mandir umbrella. `final-summary.js` is intentionally pinned to `TEST_MODE=false` in the workflow env and is the last remaining consumer that still reads the legacy Google Sheet CSV (`GOOGLE_SHEET_ID`). The Telegram bot's `/summary[suffix]` command (see Telegram Bot) dispatches the same `rsvp-summary.yml` workflow with `trigger_chat_id` so the reply lands in whichever chat ran the command.

There is no longer a Sheet ↔ Supabase parity-check job — the former `sync-check` workflow/script was removed when the Sheet mirror was retired.
...
| `POST` (single or batch) | `create_delete_passes` | logs `pass.create` |
| `PATCH` `{ id, guest_name?, phone? }` | `edit_passes` | re-checks per-event phone uniqueness; logs `pass.edit` |
| `DELETE` `{ id }` or `{ ids: [...] }` | `create_delete_passes` | hard delete; logs `pass.delete` |
...
`blocked` and `watching` are computed in JS over the 60-min window with whitelisted IPs filtered out. The two write methods upsert and delete on `invite_ip_whitelist` (PK `ip`) and log `ip.whitelist` / `ip.unwhitelist` to the audit trail. `added_by` is always written as `"superadmin"` (both the password and superadmin-session auth paths collapse to the same label).

#### Note: no rate limit on admin login

`netlify/functions/manager-auth.js` (the admin/manager login endpoint) has **no IP throttle and no failed-attempt counter**. The rate-limit table is exclusively for the public `/invite` lookup. Admin login protection relies on the long, server-stored `ADMIN_PASSWORD` and on bcrypt-hashed manager passwords (see Auth & Security). Login *attempts* are, however, recorded (success and failure) in the audit log — see Auth & Security › Audit logging.
...
- **Toggle enforcement.** `submit-rsvp.js` reads the `rsvp_settings` row for both `'global'` and the requested `zone` in one query. If either row exists with `enabled: false` the request is rejected `403 "RSVPs are currently closed for this event."` This prevents a stale browser tab from slipping past a manager turning RSVPs off (see Netlify Functions / `manage-rsvp.js`).
- **Input bounds.** `guests` is clamped to `Math.max(1, parseInt(...))` and hard-rejected over 100. `name` and `zone` are trimmed and required.
- **No phone, no email, no IP logged.** Submissions write `{id, zone, name, guests, event_name, submitted_at, sheet_row_id}` to Supabase `rsvps` and nowhere else — the Google Sheet mirror has been retired, so there is no second write. (`sheet_row_id` is just a legacy name for the row's own UUID; see RSVP flow.)
...
**(a) `x-admin-password` header** — accepted by `admin-passes.js`, `admin-rate-limit.js`, `backfill-rsvps.js`, `get-archived-rsvps.js`, `get-rsvps.js`, `manage-rsvp.js`, `restore-rsvps.js`, `superadmin-audit-log.js`, `superadmin-events.js`, `superadmin-managers.js`. Each does a literal `=== process.env.ADMIN_PASSWORD` compare, e.g. `admin-rate-limit.js` lines 12–14:
...
**(b) Login path through `/login`** — the UI submits `{ username: 'admin', password }` to `/.netlify/functions/manager-auth/login`. `manager-auth.js` compares plaintext, and on match mints a `crypto.randomBytes(32).toString('hex')` token, inserts a row into `manager_sessions` with `manager_id = null` (the marker for superadmin) and a 24-hour `expires_at`, then returns `{ token, username:'admin', role:'superadmin', permissions: SUPERADMIN_PERMISSIONS }`. A successful login (and every failure, with a `reason`) is recorded in the audit log.
...
**`/superadmin` is just a 302** to `/admin` (`netlify.toml`). There is no distinct page or function suite — the role is a flag, the UI conditionally renders extra tabs (Managers, Events, Flyers full controls, Invite Lookup Security, Audit Log) when `localStorage.admin_role === 'superadmin'`, and the functions re-check server-side.
...
CRUD lives in `netlify/functions/superadmin-managers.js`, superadmin-gated (and audited via `manager.create` / `manager.update` / `manager.delete`). Create and PATCH hash via `bcrypt.hash(password, 10)`. DELETE pre-deletes `manager_sessions WHERE manager_id = id` before deleting the row (FKs are not `ON DELETE CASCADE`).

**Login** in `manager-auth.js`:
...
Failure returns the same `"Invalid username or password"` string for missing user and bad password — no user-enumeration leak (though the audit log distinguishes `unknown_user` from `bad_password` internally).

**Session validation on subsequent calls** — the token is sent in a custom header `x-manager-token`. The browser stores it in `localStorage` (see `public/admin/index.html`):
...
**Logout** POSTs the token to `/.netlify/functions/manager-auth/logout`, which hard-deletes the row (and logs `logout`).
...
`netlify/functions/telegram-webhook.js` is registered with Telegram via `setWebhook` with the `secret_token` parameter. Telegram then sends that string back in the `X-Telegram-Bot-Api-Secret-Token` header on every callback. The function does a **constant-time** compare:
...
Mismatches and non-POSTs both return `200` with empty body so attackers can't fingerprint the endpoint. Beyond the secret, most admin commands (`/uploadflyer`, `/removeflyer`, `/cancel`, `/refreshall`, all `callback_query` handling including `review:approve` / `review:reject`) further check `chatId === process.env.TELEGRAM_CHAT_ID` so even a leaked secret can't drive admin actions from a non-admin chat. Flyer approve/reject via the webhook are written to the audit log with the Telegram actor recorded explicitly (`actor_id: 'tg:<from.id>'`).
...
```js
// scripts/automate.js — push step (5-attempt retry loop)
...
Write access is granted by the workflow's job-level permissions block in `.github/workflows/rsvp-automation.yml`:
...
Without that, `actions/checkout@v5` would inject a read-only token and the push would fail with `403`. The `dist/` push is wrapped in a **5-attempt retry loop**: on rejection it runs `git pull --rebase --autostash -X theirs origin main` between attempts (favoring the remote's copy of regenerated files, `--autostash` so the rebase doesn't choke on the in-flight `deadlines.json` commit) and retries, giving up only after 5 tries. Concurrent runs are further serialized by the workflow's `concurrency` group (see Build & Deployment).
...
### Audit logging — every login and state-change is recorded

Because authorization lives entirely in function code (§9), the project keeps a server-side **audit trail** of who did what, when — independent of any external logging service.

- **Table `audit_log`** (migration `scripts/migrations/2026-08-28-add-audit-log.sql`, run once in the Supabase SQL editor). Columns: `id` (bigserial PK), `created_at`, `actor_id` (managers.id as text — or `null`/`tg:<id>` for superadmin / a Telegram actor), `actor_name` (`'admin'` for superadmin, or the manager username), `is_superadmin`, `action` (e.g. `login`, `flyer.upload`, `rsvp.delete`), `target` (the object acted on — zone, id, username…), `details` (`jsonb`), `success`, `ip`, `user_agent`. RLS is **enabled with no policies**, so — like every other table — only the service-role key (the Netlify functions) can read or write it. No anon/public access.
- **Helper `netlify/functions/_audit.js`** exports `logAudit(supabase, event, { action, target, details })`. It is deliberately **fire-and-forget**: it never `await`s and never throws, so a logging failure — including the table not existing yet — can never break or slow the action being audited. It derives the actor **in the background from the same auth credential the request already carries** (`x-admin-password` matching `ADMIN_PASSWORD` → superadmin; `x-manager-token` → the `manager_sessions` row → manager id, then the `managers` row for the username; `x-builder-session` → the `builder_sessions` row) and stamps the row with the resolved username plus the request's `ip` (`x-nf-client-connection-ip` → `x-forwarded-for[0]` → `client-ip`) and `user_agent` (capped 400 chars). Login is the one caller that passes an explicit `actorName` / `isSuperadmin`, since no session row exists yet.
- **Hooks.** `manager-auth.js` logs every login **success and failure** (with a `reason` — `bad_password` / `unknown_user`) plus logout. Roughly a dozen state-changing functions call `logAudit` for their mutation: `flyer.upload` / `flyer.remove` / `flyer.remove_bulk` (`manage-flyers.js`), `flyer.refresh` (`refresh-flyers.js`), `flyer.submit_for_review` (`review-flyer.js`), `rsvp.edit` / `rsvp.delete` (`get-rsvps.js`), `rsvp.toggle` (`manage-rsvp.js`), `pass.create` / `pass.edit` / `pass.delete` (`admin-passes.js`), `event.create` / `event.update` / `event.delete` (`superadmin-events.js`), `event.rename` (`update-event-name.js`), `manager.create` / `manager.update` / `manager.delete` (`superadmin-managers.js`), `ip.whitelist` / `ip.unwhitelist` (`admin-rate-limit.js`), `builder.open` (`create-builder-session.js`), and `site.redeploy` (`trigger-redeploy.js`). `telegram-webhook.js` logs `flyer.approve` / `flyer.reject`, passing the Telegram actor explicitly (`actor_id: 'tg:<from.id>'`) because a callback carries no portal credential.
- **Reader `netlify/functions/superadmin-audit-log.js`** — a **GET-only, superadmin-only** endpoint (accepts a superadmin `manager_sessions` token with `manager_id === null`, or `x-admin-password`). It selects the most recent rows (`?limit=`, default 200, capped 500) newest-first and never mutates anything.
- **Admin portal tab.** `public/admin/index.html` renders a superadmin-only **📜 Audit Log** tab (hidden for regular managers, re-checked server-side) as a table of When / Who / Action / Target / Result / **Device**. The Device column turns the raw `user_agent` into a friendly label via a client-side `deviceLabel()` — e.g. `"iPhone · Safari"`, `"Mac · Chrome"`, `"Android phone · Firefox"` — and puts the exact **IP + full user-agent on hover** (browsers don't expose a real device name, so the OS + browser label is the closest automatic option).
...
- **Slug → URL mapping.** Para-sabha-style keys (`scranton`, `mountain-top`, `moosic`, `bloomsburg`, `satsang-sabha`) are served at `/<key>/`. Mandir-slot keys (`mandir-1` … `mandir-5`) are served at `/mandir/<N>/` — the slug-to-URL rewrite happens inside `scripts/automate.js` (`MANDIR_SLOTS.includes(zone) ? '/mandir/' + zone.replace('mandir-', '') : '/' + zone`). The flat repo-level slug (`mandir-1`) is still what `deadlines.json`, the RSVP page's `RSVP_ZONE` constant, and every Netlify function expects in API calls.
- **Mandir Telegram chat is shared.** All six Mandir-style zones (`satsang-sabha`, `mandir-1`..`mandir-5`) route to the single `TELEGRAM_CHAT_ID_MANDIR` chat — see the `ZONE_CHAT_IDS` map in `netlify/functions/late-rsvp.js` and `MANDIR_ZONES` in `scripts/automate.js`.
...
- **`eventDate`** — ISO `YYYY-MM-DD` of the event itself. This is the only field the cleanup workflow (`scripts/cleanup-past-rsvps.js`) compares against `todayInNY()` to decide whether to archive RSVPs, and the only field `scripts/summary.js` uses to decide whether the event is still "active" enough to summarize. Empty string means the slug is exempt from those date-gated jobs. Derived by `scripts/automate.js` from `eventInfo.date` plus the current year, with a +1 year bump if the parsed date is more than ~3 months in the past.
...
1. `scripts/automate.js` walks every zone folder under `flyers/`, OCRs each new/changed flyer with Claude (or reuses the cached entry if unchanged), resolves the canonical event name against Supabase `zone_events` (anchored by the flyer's hash — see next subsection), computes `eventDate` from the OCR'd `date`, and assembles a fresh per-zone object.
...
3. The workflow then commits the file with message `Auto-update deadlines.json [skip ci]`. The `[skip ci]` tag stops the commit from re-triggering the workflow.
...
### Event-name overrides and `flyer_hash` anchoring

The one field an admin can override without re-uploading a flyer is the **event name**, via the admin portal (Settings → "Zone Event Name" → `netlify/functions/update-event-name.js`), which upserts `{ zone, event_name }` into the Supabase `zone_events` table. On the next deploy, `scripts/automate.js` `resolveEventName()` decides whether to honor that override or fall back to the flyer's fresh OCR name.

The subtlety: an override is meant to apply **only to the flyer it was set for**. Historically the stored name was keyed by zone alone, so a rename (e.g. "Janmashtami") would linger onto the *next*, unrelated event uploaded to the same zone. That is fixed by anchoring each stored name to a **SHA-1 of the flyer's bytes**:

- `zone_events` gained a **`flyer_hash`** column (migration `scripts/migrations/2026-08-28-add-flyer-hash.sql`, run once in the Supabase SQL editor).
- On every run, `automate.js` computes `flyerHash = crypto.createHash('sha1').update(flyerBytes).digest('hex')` for each zone's current flyer and passes it to `resolveEventName(supabase, zone, ocrName, flyerHash)`.
- If the stored `flyer_hash` **matches** the current flyer, the admin's override still belongs to this flyer and is kept. If it **differs**, the flyer — and therefore the event — has changed, so the stale override is discarded and the name is reset to the current flyer's OCR value (and the row re-anchored to the new hash). A missing row, or an empty stored name, seeds the OCR name anchored to the current hash.
- **Legacy rows** whose `flyer_hash` is `NULL` (name set before this feature existed) are preserved on the first hash-aware deploy and simply anchored to the current flyer, so no intentional override is lost.
- When an admin renames via `update-event-name.js`, the upsert deliberately sets **`flyer_hash = null`** — so the very next deploy re-anchors the fresh rename to whatever flyer is currently live (the `NULL`-branch "keep the name, adopt the current hash" path) rather than immediately treating it as stale.
- If the `flyer_hash` column hasn't been added yet, `resolveEventName()` degrades gracefully to the old zone-keyed behavior instead of erroring — it never wipes a name just because the migration hasn't run.

(Both this migration and the audit-log migration live under `scripts/migrations/` and are one-time, idempotent SQL files you run by hand in the Supabase SQL editor; they are not applied automatically by any workflow.)
...
- `netlify/functions/manage-flyers.js` — `VALID_ZONES` array.
- `netlify/functions/late-rsvp.js` — `ZONE_CHAT_IDS` map — map your slug to the appropriate Telegram chat env var.
...
- `scripts/automate.js` — add to `PARASABHA_ZONES` for a Para-sabha-style page, or to `MANDIR_ZONES` / `MANDIR_SLOTS` for a Mandir-style page; also extend the `zones` array in the main build loop.
...
- Wire the env var into `netlify/functions/telegram-webhook.js` — append a pair to `ZONE_CHAT_PAIRS` and a pair to the `MISSING_ZONE_ENV_VARS` warning list so the module-scope `ZONE_CHAT_MAP` learns to route bare `/getflyer` and `/summary` invocations from the new chat.
- Wire the env var into `netlify/functions/late-rsvp.js` `ZONE_CHAT_IDS` map, mapping the new slug to `process.env.TELEGRAM_CHAT_ID_<UPPER_SLUG>`.
...
- In `dist/<slug>/index.html`, add the input element under the `#rsvp-card` block and include its value in the `JSON.stringify({...})` body of the `submitRsvp()` `fetch` call (currently `{ zone: RSVP_ZONE, name, guests, eventName: RSVP_EVENT_NAME }` — see `dist/scranton/index.html` for the reference shape).
...
- The same field needs to be plumbed through `scripts/summary.js` (if it should appear in the digest) — otherwise summaries will silently ignore the new column.
...
[build]
  ...
  publish = "dist"
  ignore = "git diff --quiet \"$CACHED_COMMIT_REF\" \"$COMMIT_REF\" -- ':(exclude)flyers' ':(exclude)scripts' ':(exclude).github' ':(exclude)*.md'"
...
- `ignore = "git diff --quiet …"` — the build-skip guard (see the next subsection). It lets Netlify short-circuit a deploy when a commit changes nothing that affects the served site.
- `directory = "netlify/functions"` — Netlify auto-mounts every `*.js` in `netlify/functions/` at `/.netlify/functions/<name>`. Each function has its own dependency manifest at `netlify/functions/package.json` (`@supabase/supabase-js`, `bcryptjs`, `googleapis`), which Netlify installs automatically as part of the function bundle. `googleapis` is now only pulled in by the legacy Sheet tools (`backfill-rsvps.js`, `restore-rsvps.js`) — the live RSVP path no longer touches it. The site-level `build.sh` does NOT run `npm install` for functions — Netlify handles that based on the functions folder's own `package.json`.

### Skipping builds that don't affect the deploy

The `[build]` block carries an `ignore` command:

```toml
  ignore = "git diff --quiet \"$CACHED_COMMIT_REF\" \"$COMMIT_REF\" -- ':(exclude)flyers' ':(exclude)scripts' ':(exclude).github' ':(exclude)*.md'"
```

Netlify runs the `ignore` command before every build: **exit 0 skips the build, any non-zero exit proceeds with the build.** This diff exits 0 (skip) only when a commit's changes are confined to paths that never affect what Netlify serves:

- `flyers/` — source flyer images that the GitHub Action OCRs; they are not served directly.
- `scripts/` — the automation that runs on GitHub Actions, not on Netlify.
- `.github/` — workflow definitions.
- `*.md` — docs.

Any change to `dist/`, `netlify/` (functions), `public/`, `build.sh`, or `netlify.toml` still triggers a full build. The rule **fails safe**: an empty or invalid `$CACHED_COMMIT_REF` makes `git diff` exit 128 (non-zero) → Netlify builds anyway. In other words, uncertainty always resolves toward building.

The practical payoff is the double-commit pattern the flyer pipeline produces: a flyer-upload commit (touching only `flyers/`) is skipped, while the follow-up `deploy: update dist` commit it triggers — which touches `dist/` — still builds. That roughly halves the number of Netlify builds.

### What `build.sh` actually does

`build.sh` is a small shell script. Its only job is to copy the runtime pieces of `public/flyer-builder/` into `dist/flyer-builder/` so the deployed Flyer Builder reflects the latest source:
...
What gets synced on every deploy:

1. `public/flyer-builder/templates/` → `dist/flyer-builder/templates/` — the high-resolution PNG flyer backgrounds (one per zone, plus variants).
2. `public/flyer-builder/preview-templates/` → `dist/flyer-builder/preview-templates/` — the 1200×630 social-card PNGs.
3. `public/flyer-builder/fonts/` → `dist/flyer-builder/fonts/` — the `@font-face` files (`AddingtonCF.otf`, `GothamRegular.ttf`, `AppleSDGothicNeoH.ttf`).
4. `public/flyer-builder/base/` → `dist/flyer-builder/base/` — base layer assets composited by the builder.
5. `public/flyer-builder/swami-photos/` → `dist/flyer-builder/swami-photos/` and `swami-photos.json` — the guru/swami photo library and its manifest.
6. `public/flyer-builder/flyer-layout.json` (best-effort, `|| true` so a missing file never fails the build) and `flyer-render.js` — the shared layout config and render helper.
7. `public/flyer-builder/index.html` → `dist/flyer-builder/index.html` — the Flyer Builder SPA shell itself.
...
### Deploy races and workflow concurrency

Because both the automation pipeline and a human can push to `main`, and because `scripts/automate.js` itself commits a rebuilt `dist/` back to `main`, concurrent pushes can race. Two mechanisms keep that from corrupting a deploy:

- **The automation serializes itself.** The RSVP Automation workflow (`.github/workflows/rsvp-automation.yml`) declares a concurrency group so only one run touches `main` at a time:

  ```yaml
  concurrency:
    group: rsvp-automation-deploy
    cancel-in-progress: false
  ```

  `cancel-in-progress: false` is deliberate — a second run **queues** behind the first rather than cancelling it, so no in-flight `dist/` / `deadlines.json` commit is aborted midway.

- **The dist-push retries with a rebase that favors the incoming tree.** When `automate.js` pushes its rebuilt `dist/`, a concurrent commit can make the push non-fast-forward. Instead of failing, it retries up to five times, rebasing on `origin/main` between attempts:
  ...
  A generated `dist/` file conflicts wholesale with a concurrently-generated one, so a plain rebase would hit an unresolvable merge conflict. `-X theirs` resolves every conflict in the incoming branch's favor (during a rebase "theirs" is the upstream/`origin/main` side being replayed onto), and `--autostash` keeps an in-progress `deadlines.json` edit from blocking the rebase. If all five attempts fail, the script throws with a message pointing at a misconfigured concurrency group — that combination should be effectively impossible when the concurrency block above is in place.
...
`netlify.toml` defines five header blocks, all of them permissive CORS for the Flyer Builder's runtime assets:
...
These rules exist because the Flyer Builder canvas (`public/flyer-builder/index.html`) draws the zone template PNGs, base layers, swami photos, and renders custom-font text into a canvas. Without `Access-Control-Allow-Origin: *` on those images and the `@font-face` files, the browser would taint the canvas and refuse to call `toDataURL()` — breaking the "Send for Review" capture. The header blocks are tightly coupled to the corresponding `cp` lines in `build.sh`: the build copies the assets into `dist/flyer-builder/`, and the headers make them safe to read from canvas.
...
- [ ] **Supabase schema**: the schema is NOT checked in as a bootstrap file. You must create every table from Supabase Schema by hand in the Supabase Dashboard SQL editor (or recreate them from a snapshot of an existing SCREvents project). At minimum: `rsvps`, `rsvps_archive`, `rsvp_settings`, `managers`, `manager_sessions`, `events`, `vip_passes`, `flyer_reviews`, `builder_sessions`, `zone_events`, `telegram_upload_sessions`, `invite_lookup_attempts`, `invite_ip_whitelist`, `audit_log`. Add the implied indexes (`manager_sessions(token)` unique, `invite_lookup_attempts(ip, attempted_at)`, `vip_passes(phone_normalized)`, `rsvps(sheet_row_id)`). Then run the two SQL migration files under `scripts/migrations/` — `2026-08-28-add-flyer-hash.sql` (adds `zone_events.flyer_hash`) and `2026-08-28-add-audit-log.sql` (creates the `audit_log` table with RLS on, service-role only). Both are idempotent.
...
- [ ] **(Optional / legacy) Google Sheet + service account**: only needed if you intend to run the retired Google Sheets tooling (`cleanup-past-rsvps.js`, `restore-rsvps.js`, `backfill-rsvps.js`, `final-summary.js`). The live RSVP path is Supabase-only and never touches the Sheet. If you want the legacy tools, create a Sheets project with a `responses` tab, a Google Cloud service account with Sheets API access + JSON key, share the sheet with the service-account email, and save the sheet ID as `GOOGLE_SHEET_ID` and the JSON key contents (one-line) as `GOOG_SA_JSON`.
...
> **Google Sheet retired.** The live RSVP path is Supabase-only. `submit-rsvp.js` and `get-rsvps.js` write and read Supabase exclusively — there is no Sheet mirror, merge, or dual-write. Consequently `GOOGLE_SHEET_ID`, `GOOG_SA_JSON`, and `GOOGLE_SERVICE_ACCOUNT_JSON` are **legacy / optional**: they are referenced only, best-effort, by the retired-Sheet tools (`backfill-rsvps.js`, `restore-rsvps.js`, `cleanup-past-rsvps.js`, `final-summary.js`) and are not required to run the site. Note that the `rsvps` table still has a column named `sheet_row_id` — that is now just a legacy *name* for the row's own UUID, used as a cross-key; the admin API likewise still uses the field name `powerapps_id` (= `sheet_row_id`) in requests/responses. Both names are retained even though there is no live PowerApps or Google Sheet integration.

### Master table (alphabetical)

| Name | Required | Where set | Consumers | Purpose | How to obtain |
|---|---|---|---|---|---|
| `ADMIN_PASSWORD` | Yes (for admin login) | Netlify | `netlify/functions/admin-passes.js`, `netlify/functions/admin-rate-limit.js`, `netlify/functions/backfill-rsvps.js`, `netlify/functions/get-archived-rsvps.js`, `netlify/functions/get-rsvps.js`, `netlify/functions/manage-rsvp.js`, `netlify/functions/manager-auth.js`, `netlify/functions/restore-rsvps.js`, `netlify/functions/superadmin-audit-log.js`, `netlify/functions/superadmin-events.js`, `netlify/functions/superadmin-managers.js` | The plaintext password that, when sent as `username=admin` to `/.netlify/functions/manager-auth/login`, mints a superadmin session (a `manager_sessions` row with `manager_id IS NULL`). Also accepted as the `x-admin-password` header on a handful of superadmin-only endpoints (including the audit-log reader). Never sent to the browser. | Generate a long random string: `openssl rand -hex 32`. Paste it into the Netlify env var UI. Share with trusted admins out-of-band only. |
| `ANTHROPIC_API_KEY` | Yes (for flyer OCR) | GitHub Actions | `scripts/automate.js`, `.github/workflows/rsvp-automation.yml` | Authenticates calls to the Anthropic Messages API (`claude-sonnet-4-6`, vision) that OCR each flyer image into structured event metadata. | Anthropic Console → https://console.anthropic.com → `Settings → API Keys → Create Key`. Add billing credit to the workspace first; OCR will hard-fail on an unfunded key. |
...
| `GOOGLE_SERVICE_ACCOUNT_JSON` | No (legacy / optional) | GitHub Actions | `scripts/cleanup-past-rsvps.js`, `.github/workflows/cleanup-past-rsvps.yml`, `.github/workflows/rsvp-automation.yml` (passed through, but no longer read by `automate.js`) | Alternate variable name for the Google service-account JSON, used only by the retired-Sheet tooling. `cleanup-past-rsvps.js` falls back to this if `GOOG_SA_JSON` is unset. The live RSVP path never touches it. | Same as `GOOG_SA_JSON` below — populate one of the two only if you keep the legacy Sheet-delete step alive. |
| `GOOGLE_SHEET_ID` | No (legacy / optional) | Both (legacy) | `netlify/functions/backfill-rsvps.js`, `netlify/functions/restore-rsvps.js`, `scripts/cleanup-past-rsvps.js`, `scripts/final-summary.js`, `.github/workflows/cleanup-past-rsvps.yml`, `.github/workflows/final-summary.yml` | ID of the **retired** Google Sheet. The live RSVP path (`submit-rsvp.js` / `get-rsvps.js`) is Supabase-only — there is no dual-write, mirror, or merge. This var survives only for the best-effort legacy tools: Sheet backfill/restore, the event-night cleanup's Sheet-delete step, and the `final-summary.js` CSV pull. Leave it unset if you do not run those tools. | Open the sheet in Google Drive; copy the segment between `/d/` and `/edit` in the URL: `docs.google.com/spreadsheets/d/<SHEET_ID>/edit`. |
| `GOOG_SA_JSON` | No (legacy / optional) | Both (legacy) | `netlify/functions/backfill-rsvps.js`, `netlify/functions/restore-rsvps.js`, `scripts/cleanup-past-rsvps.js`, `.github/workflows/cleanup-past-rsvps.yml` | Full JSON payload of a Google Cloud service account with access to `GOOGLE_SHEET_ID`. Authenticates Sheets API v4 reads/appends/`batchUpdate` deleteDimension calls made by the retired-Sheet tools only. The live site does not read it. | Google Cloud Console → `IAM & Admin → Service Accounts → Create` → grant no IAM roles (Sheets is share-based) → `Keys → Add key → Create new key → JSON`. Paste the entire JSON (as a single line, including curly braces) as the env var value. Then open the sheet and `Share` it with the service account's `client_email` as Editor. Enable the Google Sheets API on the project. |
...
| `SUPABASE_SERVICE_KEY` | Yes | Both | `netlify/functions/admin-passes.js`, `netlify/functions/admin-rate-limit.js`, `netlify/functions/backfill-rsvps.js`, `netlify/functions/create-builder-session.js`, `netlify/functions/get-archived-rsvps.js`, `netlify/functions/get-pass.js`, `netlify/functions/get-rsvps.js`, `netlify/functions/lookup-pass.js`, `netlify/functions/manage-flyers.js`, `netlify/functions/manage-rsvp.js`, `netlify/functions/manager-auth.js`, `netlify/functions/refresh-flyers.js`, `netlify/functions/restore-rsvps.js`, `netlify/functions/review-flyer.js`, `netlify/functions/rsvp-status.js`, `netlify/functions/submit-rsvp.js`, `netlify/functions/superadmin-audit-log.js`, `netlify/functions/superadmin-events.js`, `netlify/functions/superadmin-managers.js`, `netlify/functions/telegram-webhook.js`, `netlify/functions/trigger-redeploy.js`, `netlify/functions/update-event-name.js`, `netlify/functions/verify-builder-session.js`, `scripts/automate.js`, `scripts/cleanup-past-rsvps.js`, `scripts/summary.js`, `scripts/sync-check.js`, `.github/workflows/cleanup-past-rsvps.yml`, `.github/workflows/rsvp-automation.yml`, `.github/workflows/rsvp-summary.yml`, `.github/workflows/supabase-keepalive.yml`, `.github/workflows/sync-check.yml` | Service-role key that bypasses RLS. The entire backend assumes this — see Supabase Schema for why every table (including the new `audit_log`) is service-role-only. Treat as a root credential; leakage = total compromise. | Supabase → `Project Settings → API → Project API keys → service_role secret`. Set in BOTH Netlify and GitHub Actions. Never paste into client code or commit. |
...
`SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_CHAT_ID_SCRANTON`, `TELEGRAM_CHAT_ID_MOUNTAIN_TOP`, `TELEGRAM_CHAT_ID_MOOSIC`, `TELEGRAM_CHAT_ID_BLOOMSBURG`, `TELEGRAM_CHAT_ID_MANDIR`.

**Legacy / optional** (retired Google Sheet tooling — NOT needed for the live site; set only if you run the legacy backfill/restore/cleanup/final-summary tools):
`GOOGLE_SHEET_ID`, `GOOG_SA_JSON`, `GOOGLE_SERVICE_ACCOUNT_JSON`.
...
- `/.netlify/functions/submit-rsvp` writes to Supabase `rsvps`. There is no Google Sheet dual-write — Supabase is the sole store.
...
- No flyer OCR / `dist/` rebuild pipeline — add `ANTHROPIC_API_KEY` + `GITHUB_PAT`.
...
- The retired Google Sheet tooling (`backfill-rsvps`, `restore-rsvps`, `final-summary`'s CSV pull, `cleanup-past-rsvps`' Sheet-delete step) stays dormant — add `GOOGLE_SHEET_ID` + `GOOG_SA_JSON` only if you actually run those legacy tools. The live site never needs them.

Production parity adds (in priority order): `GITHUB_PAT`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY`, then the five `TELEGRAM_CHAT_ID_*` per-zone variables. (`GOOGLE_SHEET_ID` / `GOOG_SA_JSON` remain optional — legacy-Sheet-tool only.)
...
- Google Cloud (**OPTIONAL / legacy** — only if you plan to run the retired Google Sheets tooling: `cleanup-past-rsvps.js`, `restore-rsvps.js`, `backfill-rsvps.js`, or `final-summary.js`. The live site, the RSVP path, and the flyer/OCR pipeline do NOT need it — RSVPs are Supabase-only.)
...
3. Open SQL Editor and paste the schema bootstrap. The repo has no `.sql` bootstrap file (see Supabase Schema for the rationale), so you create the base tables now. Here is a single transactional script that reproduces every base table the code expects:
...
create table public.rsvps (
  ...
  sheet_row_id    text   -- legacy NAME for the row's own UUID; used as a cross-key. Keep it.
);
...
4. **Apply the schema migrations (required).** Two files under `scripts/migrations/` extend the base schema above and MUST be run once, in the Supabase SQL Editor (Dashboard → SQL Editor), on a fresh project. Both are idempotent (`if not exists`), so re-running them is harmless:

   - `scripts/migrations/2026-08-28-add-flyer-hash.sql` — adds a `flyer_hash text` column to `zone_events`. `scripts/automate.js` (`resolveEventName`) uses it to anchor an admin's event-name override to a SHA-1 of the flyer bytes: while the stored hash matches the current flyer the override name is kept, but a replaced flyer resets the name to the fresh OCR value, so an override never lingers onto the next event uploaded to that zone. (`update-event-name.js` sets `flyer_hash = null` on rename so the next deploy re-anchors.)
   - `scripts/migrations/2026-08-28-add-audit-log.sql` — creates the `audit_log` table (`id, created_at, actor_id, actor_name, is_superadmin, action, target, details jsonb, success, ip, user_agent`), with RLS enabled and no policies so **only** the service-role key can read/write it. It is written fire-and-forget by `netlify/functions/_audit.js` (from `manager-auth.js` logins/logout, ~12 state-changing functions, and `telegram-webhook.js` flyer approve/reject) and read by the superadmin-only `netlify/functions/superadmin-audit-log.js` (surfaced in the admin portal's "Audit Log" tab).

   ```bash
   # From the repo root — paste each file's contents into the Supabase SQL editor:
   cat scripts/migrations/2026-08-28-add-flyer-hash.sql
   cat scripts/migrations/2026-08-28-add-audit-log.sql
   ```

5. About Row Level Security: every server-side caller (Netlify Functions + GitHub Actions scripts) authenticates with `SUPABASE_SERVICE_KEY`, which bypasses RLS. You can leave RLS in its default state (the `audit_log` migration turns it on for that one table). If you want to be defensive, enable RLS on every table without adding any policies — service-role still bypasses it, and you eliminate any chance of accidental anon access.

6. Create the Storage bucket used by the flyer-builder review pipeline:
...
### 5. Anthropic API key (+ optional legacy Google service account)

1. Anthropic Console -> Settings -> API Keys -> Create Key. Save as `ANTHROPIC_API_KEY`. Used by `scripts/automate.js` for flyer OCR. **Required** for the flyer pipeline.
2. **(OPTIONAL — legacy Sheet tooling only.)** The following Google steps are needed *only* if you plan to run the retired Google Sheets tools (`cleanup-past-rsvps.js`, `restore-rsvps.js`, `backfill-rsvps.js`, `final-summary.js`). The live site never reads the Sheet — skip this block otherwise.
   - Google Cloud Console -> create a project -> enable the Google Sheets API -> IAM & Admin -> Service Accounts -> Create -> Keys -> Add key -> JSON. Download the JSON.
   - Create a Google Sheet (any blank sheet works). Copy the ID from the URL `docs.google.com/spreadsheets/d/<SHEET_ID>/edit` and save as `GOOGLE_SHEET_ID`. Share the sheet with the service account email (`client_email` in the JSON) with Editor access.
   - The contents of the JSON file go into two env-var aliases — `GOOG_SA_JSON` (used by the legacy Netlify Functions) and `GOOGLE_SERVICE_ACCOUNT_JSON` (used by some GitHub Actions workflows). They are the same value; set both. Paste as a single-line string when filling the dashboards.
...
(`GOOGLE_SHEET_ID` and `GOOG_SA_JSON` are optional/legacy — add them only if you run the retired Sheet tools.)
...
TELEGRAM_CHAT_ID_BLOOMSBURG
TELEGRAM_CHAT_ID_MANDIR
GOOGLE_SHEET_ID            # optional — legacy Sheet tooling only
GOOG_SA_JSON               # optional — legacy Sheet tooling only
GOOGLE_SERVICE_ACCOUNT_JSON # optional — legacy Sheet tooling only
...
3. Verify each workflow boots by manually dispatching it once (Actions -> select workflow -> Run workflow):
   ...
   - `sync-check.yml` -> green = Supabase readable (the RSVP drift check is now Supabase-only)
...
3. Smoke-test:
   ...
   - Sign in as `admin` / `ADMIN_PASSWORD` -> should land on `/admin` with every tab visible (including the superadmin-only **Audit Log** tab)
...
3. The push triggers `.github/workflows/rsvp-automation.yml`, which runs `scripts/automate.js`. The script OCRs the flyer with Claude, writes the OCR'd event name into `zone_events` (anchored to the flyer's `flyer_hash`), regenerates `dist/scranton/index.html` plus `dist/np/scranton/index.html` plus `og.jpg`, updates `deadlines.json`, and pushes `dist/` back to `main`. Netlify picks up the push and redeploys.
...
- Pick the zone, fill the form, drag overlays, click Send for Review. (The on-screen draft shows a tiled diagonal "PREVIEW" watermark overlay; the published `flyer.jpg` / `og.jpg` carry none — they are exported from a separate offscreen canvas.)
...
3. Pick the zone, fill in event details, click **Send for Review**. (The draft on screen shows a tiled diagonal "PREVIEW" overlay; the exported/published flyer carries no watermark — it is rendered from a separate offscreen canvas.)
...
5. An admin (only `TELEGRAM_CHAT_ID` honors the callback) taps **Approve**. `netlify/functions/telegram-webhook.js` atomically claims the row (`pending → processing`), deletes any existing `flyers/<zone>/*.jpg|jpeg|png` files in the `pritpnp/rsvp-automation` repo, commits `flyers/<zone>/flyer.jpg` and `flyers/<zone>/og.jpg`, then marks the row `approved` (and logs `flyer.approve` to the audit log).
...
**Path B — Direct upload from admin portal.** Managers with `permissions.upload_flyers` can use the **Flyers** tab in `/admin`, which calls `netlify/functions/manage-flyers.js`. This skips the review queue and commits straight to `pritpnp/rsvp-automation` with message `Upload flyer for <zone> via admin portal`, then deletes the stale `og.jpg` so `scripts/automate.js` regenerates it on the next workflow run. The admin SPA compresses the image client-side (canvas, longest edge ≤ 2200px, JPEG) before POSTing to stay under Netlify's ~6MB function body limit, and `manage-flyers.js` wraps its handler in a top-level try/catch plus an explicit `413` guard that returns `Image too large — please use a smaller flyer image (under ~4MB).` if a raw payload still slips through.
...
- **Fix:**
  ...
  2. The minimum production set for the Netlify function layer is: `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `ADMIN_PASSWORD`, `GITHUB_PAT`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_CHAT_ID_SCRANTON`, `TELEGRAM_CHAT_ID_MOUNTAIN_TOP`, `TELEGRAM_CHAT_ID_MOOSIC`, `TELEGRAM_CHAT_ID_BLOOMSBURG`, `TELEGRAM_CHAT_ID_MANDIR`, `TELEGRAM_WEBHOOK_SECRET`. (`GOOGLE_SHEET_ID` / `GOOG_SA_JSON` are NOT part of the minimum set — they are legacy/optional, needed only by the retired Sheet tools.)
...
- **Fix:**
  ...
  2. For `500 Could not save your RSVP.`: verify `SUPABASE_URL` and `SUPABASE_SERVICE_KEY` in Netlify env match the current Supabase project (Project Settings → API). Rotate if needed and redeploy. (There is no Google Sheet dual-write to rule out — `submit-rsvp.js` writes Supabase only.)
  3. For schema issues, run a `SELECT count(*) FROM rsvps;` in the Supabase SQL editor. If it errors with `relation "rsvps" does not exist`, the table needs to be created — see **Supabase Schema** for the columns. There is no migrations file for the base tables; they were created manually in the Supabase dashboard (the two files under `scripts/migrations/` only add `zone_events.flyer_hash` and the `audit_log` table on top).
...
- **Fix:**
  ...
  3. The full set required by GitHub Actions secrets: `ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_CHAT_ID_SCRANTON`, `TELEGRAM_CHAT_ID_MOUNTAIN_TOP`, `TELEGRAM_CHAT_ID_MOOSIC`, `TELEGRAM_CHAT_ID_BLOOMSBURG`, `TELEGRAM_CHAT_ID_MANDIR`. (`GOOGLE_SHEET_ID`, `GOOG_SA_JSON` / `GOOGLE_SERVICE_ACCOUNT_JSON` are optional — only the legacy Sheet workflows `cleanup-past-rsvps.yml` and `final-summary.yml` reference them.)
...
- **Cause:** Someone edited `dist/flyer-builder/index.html` (or files under `dist/flyer-builder/templates|preview-templates|fonts|base|swami-photos/`) directly instead of editing the source-of-truth at `public/flyer-builder/`. `build.sh` runs on every Netlify deploy and overwrites `dist/flyer-builder/` from `public/flyer-builder/`, so direct edits to `dist/` will be silently clobbered on the next deploy.
- **Fix:**
  1. Move any intended edits back to `public/flyer-builder/`. The Netlify build script `build.sh` copies `templates/`, `preview-templates/`, `fonts/`, `base/`, `swami-photos/` (plus `swami-photos.json`, `flyer-layout.json`, `flyer-render.js`, and `index.html`) from `public/flyer-builder/` into `dist/flyer-builder/` on every deploy.
  ...
  4. To prevent recurrence: when adding a new flyer-builder asset, drop it in `public/flyer-builder/<subdir>/` and add the corresponding `cp` line to `build.sh` if it's a new top-level subdir. Asset paths the SPA fetches at runtime (`/flyer-builder/templates/*`, `/flyer-builder/preview-templates/*`, `/flyer-builder/fonts/*`, `/flyer-builder/swami-photos/*`, `/flyer-builder/base/*`) are CORS-enabled via the `[[headers]]` blocks in `netlify.toml` — new subdirs would also need a CORS header entry.
