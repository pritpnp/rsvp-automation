...
<body>
    ...
    <div id="rsvp-open">
      ...
      <div style="background:#fff;border-radius:20px;padding:28px 24px;box-shadow:0 4px 24px rgba(92,45,10,0.10);">
        <div id="rsvp-form" style="text-align:left;">
          <input id="rsvp-name" type="text" placeholder="Full Name" autocomplete="name" oninput="this.value=this.value.replace(/[^a-zA-Z '.-]/g,'')" style="width:100%;padding:13px 14px;border:1px solid #e0d5c8;border-radius:12px;font-size:15px;margin-bottom:10px;box-sizing:border-box;font-family:'DM Sans',sans-serif;background:#FDF6EC;color:#5C2D0A;" />
    ...
    <div id="rsvp-closed" style="display:none;">
      <div style="background:#fff;border-radius:20px;padding:32px 24px;text-align:center;box-shadow:0 4px 24px rgba(92,45,10,0.10);">
        ...
        <div id="late-form" style="text-align:left;">
          <input id="late-name" type="text" placeholder="Full Name" oninput="this.value=this.value.replace(/[^a-zA-Z '.-]/g,'')" style="width:100%;padding:12px 14px;border:1px solid #e0d5c8;border-radius:12px;font-size:15px;margin-bottom:10px;box-sizing:border-box;font-family:'DM Sans',sans-serif;" />
  ...
  <script>
    ...
    async function fetchFlyerBlob() {
      var res = await fetch(FLYER_URL);
      return await res.blob();
    }
    async function shareFlyerMobile() {
      var msg = document.getElementById('share-msg');
      try {
        var blob = await fetchFlyerBlob();
        var file = new File([blob], 'flyer.jpg', { type: 'image/jpeg' });
        ...
        if (e.name !== 'AbortError') {
          ...
          msg.textContent = '\u274C Could not share. Try saving the image manually.';
        }
      }
    }
    async function copyFlyerDesktop() {
      var msg = document.getElementById('share-msg');
      try {
        var blob = await fetchFlyerBlob();
...
async function deployAllToNetlify(pages, deadlines = {}, eventInfoMap = {}) {
  ...
  // Add static pages: VIP pass, login, admin, invite lookup
  const staticPages = [
...
async function main() {
  ...
  for (const zone of zones) {
    ...
    const files = fs.readdirSync(zoneDir).filter(f => /.(png|jpg|jpeg)$/i.test(f) && f !== "og.jpg");
    if (files.length > 0) {
