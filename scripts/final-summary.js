...
function download(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    https.get(url, handleResponse).on('error', err => { fs.unlink(dest, () => {}); reject(err); });
  });
}
...
async function main() {
  ...
  for (const [zone, info] of Object.entries(deadlines)) {
    const { eventDate, eventName, date } = info;
    if (!eventDate) continue;

    const isEventDay = TEST_MODE || eventDate === today;
    if (!isEventDay) {
      console.log(`Skipping ${zone} — event date ${eventDate} is not today`);
      continue;
    }
    ...
    if (zoneResponses.length === 0) {
      console.log(`No RSVPs for ${zone} — skipping final summary`);
      continue;
    }
  ...
  if (sent === 0) console.log('No events ending today — no final summaries sent');
}
